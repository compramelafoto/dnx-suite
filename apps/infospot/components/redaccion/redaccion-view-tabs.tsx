...
import { REDACCION_VISTAS, type RedaccionVista } from "@/lib/redaccion-queues";
...
export function RedaccionViewTabs({ active, counts }: Props) {
  return (
    <nav aria-label="Secciones de trabajo" className="-mx-1 overflow-x-auto px-1 pb-1">
      <ul className="flex min-w-max gap-2 sm:min-w-0 sm:flex-wrap">
        {REDACCION_VISTAS.map((vista) => {
          const isActive = active === vista.id;
          const count = counts?.[vista.id];
          return (
            <li key={vista.id}>
              <Link
                href={vista.id === "mi-trabajo" ? "/redaccion" : `/redaccion?vista=${vista.id}`}
