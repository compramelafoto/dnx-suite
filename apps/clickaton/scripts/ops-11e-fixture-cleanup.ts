/**
 * ETAPA 11E — cleanup SOLO fixture (AND estricto).
 *
 * Dry-run default. Apply:
 *   SFEF11E_ALLOW_CLEANUP=1 SFEF11E_APPLY=1 \
 *   SFEF11E_EDITION_ID=... DATABASE_URL=... \
 *   pnpm --filter @repo/db exec tsx ../../apps/clickaton/scripts/ops-11e-fixture-cleanup.ts
 */
...
const FIXTURE_EMAIL_PREFIX_RE = /^clickaton11e-/i;
...
async function main() {
  const apply = process.env.SFEF11E_ALLOW_CLEANUP === "1" && process.env.SFEF11E_APPLY === "1";
  const credsPath = process.env.SFEF11E_CREDS_PATH ?? "/tmp/clickaton-11e-fixture.env";
  const file = loadEnvFile(credsPath);
  const editionId = process.env.SFEF11E_EDITION_ID ?? file.SFEF11E_EDITION_ID;
  const contestId = process.env.SFEF11E_CONTEST_ID ?? file.SFEF11E_CONTEST_ID;
  ...
  if (!edition.slug.startsWith(SLUG_PREFIX)) {
    throw new Error("ABORT slug no es fixture 11e");
  }
  if (edition.fotorankContestId !== contestId) {
    throw new Error("ABORT contest mismatch");
  }

  // AND estricto: edición exacta + isOpsFixture + slug prefix + emails de registraciones @fotorank.test.
  ...
  if (!apply) {
    console.log(JSON.stringify({ note: "Set SFEF11E_ALLOW_CLEANUP=1 SFEF11E_APPLY=1 para borrar" }));
  ...
  const judgeId = file.SFEF11E_JUDGE_ACCOUNT_ID;
  if (judgeId) {
  ...
  const wsId = file.SFEF11E_JURY_WORKSPACE_ID;
  if (wsId) {
    await prisma.workspace.delete({ where: { id: wsId } }).catch(() => null);
  }

  // Assets / entries / participants (solo contest fixture).
  if (entryIds.length) {
  ...
  // Submissions / sync / outbox / regs / prompts / ticket / uploadConfig / edición (solo edición fixture).
  ...
  // Users fixture: solo si no tienen otras registraciones (11e- prefix + @fotorank.test).
  for (const r of regs) {
  ...
  console.log(
    JSON.stringify({
      ...
      commercialRegCountBefore: file.SFEF11E_COMMERCIAL_REG_COUNT_BEFORE,
      commercialApprovedCountAfter: commercialApprovedAfter,
      commercialApprovedCountBefore: file.SFEF11E_COMMERCIAL_APPROVED_COUNT_BEFORE,
      commercialPaidOrdersCountAfter: commercialPaidOrdersAfter,
      commercialPaidOrdersCountBefore: file.SFEF11E_COMMERCIAL_PAID_ORDERS_BEFORE,
    }),
  );
  await prisma.$disconnect();
}
