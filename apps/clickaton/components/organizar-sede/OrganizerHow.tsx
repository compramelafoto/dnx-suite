...
const { how } = organizarSedeContent;
...
export function OrganizerHow() {
  return (
    <Section
      ...
      aria-labelledby="organizer-how-title"
    >
      <Container>
        <div className="max-w-3xl">
          <p className="ck-overline text-ck-yellow">{how.eyebrow}</p>
          <h2 id="organizer-how-title" className="ck-display-lg mt-6 text-ck-text">
            {how.title}
          </h2>
          <p className="ck-body-lg mt-8 text-ck-text-secondary">{how.lead}</p>
        </div>

        <ol className="relative mt-14 space-y-0 sm:mt-16">
          {how.steps.map((step, index) => {
            const isLast = index === how.steps.length - 1;
            return (
              <li
                key={step.title}
              ...
              >
                <div className="flex flex-col items-start sm:items-center">
                  ...
                  {!isLast ? (
                    <span className="mt-2 text-ck-yellow sm:hidden" aria-hidden>
                      ↓
                    </span>
                  ) : null}
                </div>
