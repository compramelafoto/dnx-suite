...
export default async function RegistrationCredentialPage({ params }: Props) {
  ...
  const requiredCount = paid
    ? await prisma.clickatonPrompt.count({
        where: {
    ...
    : 0;
  const completedCount = registration.photoSubmissions.filter(
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8 px-4 py-12 print:py-4">
      ...
      <AccreditationLiveRedirect
        registrationId={registration.id}
        alreadyAccredited={Boolean(registration.checkIns[0])}
      />
      ...
      <Card variant="outlined" className="space-y-4 p-6">
        ...
        {registration.checkIns[0] ? (
          <div className="space-y-3 text-sm">
            ...
            <Button
              href={participantLivePath(registration.id)}
              variant="primary"
          ...
          <div className="space-y-2 text-sm text-ck-text-secondary">
            ...
            <p className="text-ck-text-muted">
              Apenas el equipo escanee tu QR, esta pantalla te lleva sola a las consignas.
            </p>
            <p>
              Kit esperado:{" "}
              {shirt
      ...
      <Card variant="yellow" className="space-y-4 p-6">
        <h2 className="font-semibold">Consignas y carga de fotos</h2>
        <p className="text-sm leading-relaxed text-ck-text-secondary">
          Las consignas viven en una sola pantalla, la misma durante todo el evento. Se publican
          todas juntas: antes de la apertura vas a ver una cuenta regresiva y, al llegar a cero,
          las consignas completas.
        </p>
        <p className="text-sm text-ck-text-muted">
          Progreso: {completedCount}/{requiredCount || "—"} enviadas.
        </p>
        <Button
          href={participantLivePath(registration.id)}
          variant="primary"
