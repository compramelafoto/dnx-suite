...
describe("updateCoursesSalesSettingsAction", () => {
  ...
  it("guarda defaultCurrency/enrollmentCtaLabel/coursesFeePercent en CourseSalesWorkspaceSettings", async () => {
  ...
  it("STAFF no puede editar el fee de cursos", async () => {
    membershipFindUniqueMock.mockResolvedValueOnce({ role: "STAFF" });
    const result = await updateCoursesSalesSettingsAction(undefined, buildFormData());
    expect(result.error).toBe("Solo owner/admin del workspace puede editar el fee de cursos.");
