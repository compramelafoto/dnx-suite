...
import { INVITATION_TTL_HOURS } from "./invitations";
 ...
 * workspace o un token. Y no promete pagos ni comprobantes: todavía no existen.
 */
...
export function buildInvitationEmailBody(input: InvitationEmailInput): {
  ...
  const hours = INVITATION_TTL_HOURS;
...
<div>
  <p>Hola ${name},</p>
  <p><strong>${institution}</strong> te invita a activar tu acceso a FotoOffice. Desde allí vas a poder acceder a la información y los servicios que la institución pone a disposición de sus socios.</p>
  ...
  <p style="font-size:13px;color:#6b7280;">El enlace vence en ${hours} horas.</p>
