...
interface OrderItemProps {
  ...
  /** Checkout álbum: no mostrar nombre de archivo; endurecer interacción con la miniatura */
  checkoutPrivacy?: boolean;
  /** Digital incluido en pack face-bulk: precio $0 en ítem; el total va en la línea del pack */
  faceBulkIncluded?: boolean;
  /** Digital incluido al comprar impresión: mostrar un solo precio combinado. */
  includesPrintDigitalBundle?: boolean;
  /** Canje preventa: ocultar bloque de precios / subtotal */
  hidePricing?: boolean;
  /** Canje preventa: solo foto, formato y acciones esenciales (sin producto/tamaño/cantidad) */
  redeemMinimalUi?: boolean;
}
...
export default function OrderItem({
  ...
  showPrintType = false,
  tertiaryColor,
  checkoutPrivacy = false,
  faceBulkIncluded = false,
  includesPrintDigitalBundle = false,
  hidePricing = false,
  redeemMinimalUi = false,
}: OrderItemProps) {
    ...
    >
      <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
        <div className="w-full sm:w-64 flex-shrink-0">
          ...
          {previewFailed ? (
            <div
              className="flex items-center justify-center text-xs text-[#6b7280] bg-[#f3f4f6] rounded"
            ...
            <img
              src={previewUrl}
              alt={checkoutPrivacy ? "Vista previa protegida" : originalName}
              loading="lazy"
              decoding="async"
              draggable={false}
              onDragStart={(e) => checkoutPrivacy && e.preventDefault()}
              onError={() => setPreviewFailed(true)}
              className={`max-w-full max-h-full object-contain ${checkoutPrivacy ? "pointer-events-auto select-none" : ""}`}
          ...
          {!checkoutPrivacy ? (
            ...
            <p className="mt-2 text-xs text-[#9ca3af] text-center">Vista previa con protección anticopia</p>
          )}
        </div>
        ...
        <div className="flex-1 space-y-4">
          ...
          {showPrintType && !isDigital && !redeemMinimalUi && (
          ...
          {!isDigital && redeemMinimalUi && (
            <p className="text-sm text-slate-600">
              Formato <strong>Impresa</strong> — se usa para beneficios de impresión del pack (tamaño y producto los
              define el pack).
            </p>
          )}
          ...
          {isDigital && redeemMinimalUi && (
            <p className="text-sm text-slate-600">
              Formato <strong>Digital</strong> — cuenta para beneficios digitales del pack.
            </p>
          )}
          {isDigital && !redeemMinimalUi && (
          ...
          {faceBulkIncluded && isDigital && !redeemMinimalUi && (
            <p className="rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-900">
              Incluida en el pack <strong>«todas las fotos donde aparecés»</strong>. No se cobra por foto: el precio del
              pack figura arriba en una sola línea.
            </p>
          )}
          ...
          {!hidePricing && (
            <div className="bg-[#f8f9fa] rounded-lg p-4 space-y-2 border border-[#e5e7eb]">
              {!isDigital && includesPrintDigitalBundle ? (
                <div className="flex justify-between items-center">
                  <span className="text-sm text-[#6b7280]">
                    {quantity > 1
                      ? `Precio total (${quantity} impresiones + digital incluido)`
              ...
              {isDigital && faceBulkIncluded && (
                <div className="flex justify-between text-sm">
                  <span className="text-[#6b7280]">Precio en este ítem:</span>
                  <span className="font-medium text-emerald-800">Incluido en el pack — {formatMoney(0)}</span>
                </div>
              )}
              {isDigital && (
                <div className="flex justify-between items-center pt-2 border-t border-[#e5e7eb]">
