...
type Props = {
  ...
  disabled?: boolean;
  /** invite: lista de invitados (evento privado). enroll: inscribe al elegir de la búsqueda. */
  mode?: "invite" | "enroll";
  onEnroll?: (photographer: PhotographerOption) => void | Promise<void>;
  enrollingUserId?: number | null;
  /** IDs a ocultar de resultados (ej. ya inscriptos). */
  excludeUserIds?: number[];
  helpText?: string;
};

export default function EventInvitePhotographers({
  value,
  onChange,
  disabled,
  mode = "invite",
  ...
  const pick = async (p: PhotographerOption) => {
    if (mode === "enroll") {
      if (onEnroll) await onEnroll(p);
      setSearch("");
      setResults([]);
      setOpen(false);
      return;
    ...
    <div className="space-y-2 w-full min-w-0">
      ...
      <div className="relative min-w-0">
        <Input
          ...
          disabled={disabled || enrollingUserId != null}
              ...
              {searching ? (
                ...
                results.map((p) => (
                  <button
                    ...
                    type="button"
                    onClick={() => void pick(p)}
                    disabled={enrollingUserId === p.id}
                    className="w-full text-left px-3 py-2 hover:bg-gray-50 border-b border-gray-100 last:border-0 text-sm disabled:opacity-60"
                  >
      ...
      <p className="text-xs text-gray-500">
        {helpText ??
          (mode === "enroll"
            ? "Elegí un fotógrafo para inscribirlo directamente en el evento. Quedará activo y podrá subir fotos."
