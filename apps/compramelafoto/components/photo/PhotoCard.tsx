...
interface PhotoCardProps {
  ...
  noDrag?: boolean;
  /** Muestra icono de foto en la esquina (galerías públicas). */
...
export default function PhotoCard({
  ...
  const primaryRole = onSelect ? "button" : undefined;
  ...
  return (
    <div
      role={primaryRole}
      tabIndex={primaryRole ? 0 : undefined}
      onClick={() => {
        onSelect?.();
      }}
      onKeyDown={primaryRole ? handleKeyDown : undefined}
    ...
    >
      ...
      <div
        ...
        onDoubleClick={(e) => {
          if (!onOpenSlide) return;
          e.stopPropagation();
          onOpenSlide();
        }}
      >
        {showMediaTypeBadge ? <GalleryMediaTypeBadgeSingle type="photo" /> : null}
        <img
          src={src}
          alt={alt}
          loading="lazy"
          decoding="async"
          className={cn(
            "max-w-full max-h-64 object-contain transition-transform",
            /* globals.css fuerza pointer-events:auto en .album-photo-container img; sin !important el toque cae en la img y en iOS a veces no dispara el click del contenedor */
      ...
      <div className="absolute top-2 right-2 flex gap-1">
        {onOpenSlide && (
          <button
            ...
            aria-label="Ver foto ampliada con marca de agua"
            title="Ver ampliada (con marca de agua)"
          >
            <svg
              className="h-5 w-5"
              fill="none"
