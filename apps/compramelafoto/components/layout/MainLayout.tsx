...
export default function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  /** Páginas públicas /album/... (preventa, etc.): sin Header/Footer del sitio, ancho real del viewport. */
  ...
  if (isHomePreview || isDesignSystemRoute || isCamOfDutyRoute) {
    return <>{children}</>;
  }
  ...
  if (maintenanceMode && !isAdminRoute && !isHomePreview) {
    return (
  ...
  return (
    <div className="min-h-screen flex flex-col w-full min-w-0">
      ...
      {!isLandPage && !isHomePreview && <Footer />}
    </div>
  );
}
