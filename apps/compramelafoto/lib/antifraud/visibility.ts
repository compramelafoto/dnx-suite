...
export interface VisibilityUser {
  ...
  role: Role;
  labId?: number;
  /** Si true (solo admin puede activarlo), el fotógrafo ve datos sensibles antes del pago en pedidos propios */
...
export function getOrderVisibilityContext(
  ...
  if (user.role === "LAB" || user.role === "LAB_PHOTOGRAPHER") {
    ...
    // Lab+foto viendo su propio pedido de impresión: no aplicar el cierre estricto de “solo lab”
    if (
      user.role === Role.LAB_PHOTOGRAPHER &&
      order.orderType === "PRINT_ORDER" &&
      order.photographerId === user.id
    ) {
      const revealSensitive =
        paymentApproved || user.allowUnpaidOrderClientData === true;
      return {
        canViewOrder: true,
        canViewCustomerData: revealSensitive,
        canViewOrderItems: revealSensitive,
        canViewFileKeys: revealSensitive,
  ...
  // Fotógrafo: pedido propio; datos sensibles si pagado o excepción admin
  const photographerLike = user.role === Role.PHOTOGRAPHER;
  const revealSensitive =
    paymentApproved ||
    (photographerLike && user.allowUnpaidOrderClientData === true);
