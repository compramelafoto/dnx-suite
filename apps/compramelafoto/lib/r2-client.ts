...
import { getPublicUrl, getR2PublicUrl, urlToR2Key } from "@/lib/r2-public-url";

export { getPublicUrl, getR2PublicUrl, urlToR2Key } from "@/lib/r2-public-url";
...
/**
 * URL pública para portada de álbum y miniaturas en listados: prioriza el JPEG **original**
 * (sin marca de agua). Si no hay `originalKey` usable, usa el preview con marca.
 */
export function coverPhotoPublicUrl(photo: {
  originalKey?: string | null;
  previewUrl?: string | null;
} | null | undefined): string | null {
  if (!photo) return null;
  const raw = photo.originalKey?.trim();
  if (raw) {
    try {
      return getR2PublicUrl(raw);
    } catch {
      // seguir con preview
    }
  }
...
/**
 * Lee un archivo desde R2 con metadata básica (un solo GetObject).
 */
export async function readFromR2Detailed(key: string): Promise<{
  buffer: Buffer;
...
export async function readFromR2(key: string): Promise<Buffer> {
  const { buffer } = await readFromR2Detailed(key);
  return buffer;
}
...
/**
 * Copia un objeto dentro del mismo bucket R2.
 */
export async function copyInR2(
  ...
  const client = getS3Client();
  const bucketName = getBucketName();
  const encodedSource = `${bucketName}/${sourceKey.split("/").map(encodeURIComponent).join("/")}`;
