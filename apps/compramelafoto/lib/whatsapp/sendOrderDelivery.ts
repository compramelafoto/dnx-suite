...
const MESSAGES = {
  ...
  packAccess: (nombre: string, linkPack: string) =>
    `Hola ${nombre} 👋\nTu compra de pack fue confirmada.\nGuardá este link: es tu acceso directo para elegir y canjear tus fotos cuando corresponda:\n${linkPack}`,
};

export function buildPreventaPackAccessWhatsAppMessage(
  nombre: string,
  linkPack: string
): string {
  return MESSAGES.packAccess(nombre, linkPack);
}

export function buildPreventaPackRecoveryWhatsAppMessage(
  nombre: string,
  links: string[]
): string {
  const list = links.map((l, i) => `${i + 1}. ${l}`).join("\n");
  return `Hola ${nombre} 👋\nTe reenviamos los accesos para continuar tu pack:\n${list}`;
}
...
export async function deliverOrderByWhatsApp(orderId: number): Promise<{
  ...
  const downloadUrl = clientToken
    ? resolveClientDigitalDownloadLinks({
        ...
        context: "whatsapp_order_delivery",
      }).primaryClientUrl
    : orderLink;
