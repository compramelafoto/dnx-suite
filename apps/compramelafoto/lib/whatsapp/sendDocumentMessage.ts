/**
 * Envía documento (ej. ZIP) por WhatsApp Cloud API.
...
export async function sendDocumentMessage(
  ...
  try {
    const document: Record<string, string> = { link: documentUrl };
    if (filename?.trim()) document.filename = filename.trim();
    ...
    if (caption && caption.trim()) {
      (body as Record<string, string>).caption = caption.trim().slice(0, 1024);
    }
