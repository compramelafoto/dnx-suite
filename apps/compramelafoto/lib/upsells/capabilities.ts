...
export function photographerCapabilitiesFromSettings(
...
export type AlbumSalesSettingsCapabilityRow = {
...
/**
 * Capabilities efectivas (puro): herencia u override por álbum.
 * - Si inherit: effective = photographerCaps - disabledCapabilities
 * - Si !inherit: effective = allowedCapabilities ∩ photographerCaps
 */
export function resolveEffectiveCapabilitySet(params: {
  photographerSettings: PhotographerSalesSettingsRow | null;
  albumSettings: AlbumSalesSettingsCapabilityRow | null;
}): Set<Capability> {
  const photographerCaps = photographerCapabilitiesFromSettings(
    params.photographerSettings
  );

  const albumSettings = params.albumSettings;
  ...
  const allowed = (albumSettings.allowedCapabilities ?? []).filter(isCapability);
...
export async function getEffectiveCapabilities(params: {
  ...
  try {
    ...
    return resolveEffectiveCapabilitySet({
      photographerSettings:
        photographerSettings as PhotographerSalesSettingsRow | null,
      albumSettings: albumSettings
        ? {
            inheritFromPhotographer: albumSettings.inheritFromPhotographer,
            allowedCapabilities: albumSettings.allowedCapabilities,
            disabledCapabilities: albumSettings.disabledCapabilities,
          }
        : null,
    });
  } catch {
