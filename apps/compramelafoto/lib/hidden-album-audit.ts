...
import { HiddenAlbumDeviceType } from "@/lib/prisma";
...
export function parseGrantCookie(cookieValue: string | null): { grantId: string; albumId: number; expiresAt: number } | null {
  ...
  try {
    ...
    if (!grantId || !albumIdStr || !expiresAtStr || !sig) return null;
...
export async function resolveHiddenAlbumVisitorAccess(
  albumId: number,
  grantCookieValue: string | null,
  prismaClient: {
    hiddenAlbumGrant: {
      findUnique: (args: {
        ...
        expiresAt: Date;
        isRevoked: boolean;
        allowedPhotoIds: unknown;
      } | null>;
    };
  }
): Promise<{ hasGrant: boolean; allowedPhotoIds: number[] | null }> {
  const parsed = parseGrantCookie(grantCookieValue);
  if (!parsed || parsed.albumId !== albumId) {
    return { hasGrant: false, allowedPhotoIds: null };
  }
  const grant = await prismaClient.hiddenAlbumGrant.findUnique({
    where: { id: parsed.grantId },
    select: { albumId: true, expiresAt: true, isRevoked: true, allowedPhotoIds: true },
  });
  if (!grant || grant.albumId !== albumId || grant.isRevoked || grant.expiresAt < new Date()) {
    return { hasGrant: false, allowedPhotoIds: null };
  }
  const allowedIds = Array.isArray(grant.allowedPhotoIds)
    ? (grant.allowedPhotoIds as number[]).filter((id) => Number.isFinite(id))
    : [];
  return { hasGrant: true, allowedPhotoIds: allowedIds };
}
