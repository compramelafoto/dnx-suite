...
/** Ancho máximo al generar preview con watermark protegido (MVP anti-captura) */
const PROTECTED_PREVIEW_MAX_WIDTH = 1200;
 ...
 */
export async function loadWatermarkImage(): Promise<Buffer | null> {
...
function createInstagramTextOverlay(
  ...
  const text = sanitizeWatermarkAscii("Compramelafoto.com - Fotoffice.com");
  ...
  const svg = `<?xml version="1.0" encoding="UTF-8"?>
    <svg width="${textWidth}" height="${textHeight}" xmlns="http://www.w3.org/2000/svg">
      <defs>${buildSvgEmbeddedFontDefs()}</defs>
      <g opacity="${WATERMARK_OPACITY}">
        <text
          ...
          stroke-opacity="0.6"
          paint-order="stroke fill">${escapeSvgText(text)}</text>
      </g>
    </svg>
  `;
...
/**
 * Regenera el JPEG de preview (misma pipeline que processPhoto: grid 3×3 + texto central).
 * El JPEG final no incluye EXIF (solo DPI/perfil para consistencia).
 */
export async function buildWatermarkedPreviewJpegFromOriginalBuffer(
  imageBuffer: Buffer,
  applyWatermark: boolean = true,
  photoId?: number
): Promise<Buffer> {
  if (applyWatermark && isProtectedPreviewWatermarkEnabled()) {
    return applyProtectedPreviewWatermark(imageBuffer, {
      photoId,
      mode: getProtectedPreviewWatermarkMode(),
      showPhotoId: photoId != null,
      maxSide: PROTECTED_PREVIEW_MAX_WIDTH,
    });
  }
  ...
  let previewBuffer = await sharp(imageBuffer)
    .rotate()
    ...
    .toBuffer();
  if (applyWatermark) {
    previewBuffer = await applyWatermarkToImage(previewBuffer);
  }
  return await sharp(previewBuffer)
    ...
    .withMetadata(withPrintMetadata(PREVIEW_DPI, iccPath))
    .toBuffer();
}
...
export async function processPhoto(
  ...
  try {
    ...
    let previewBufferProcessed: Buffer;

    if (applyWatermark && isProtectedPreviewWatermarkEnabled()) {
      previewBufferProcessed = await applyProtectedPreviewWatermark(imageBuffer, {
        mode: getProtectedPreviewWatermarkMode(),
        maxSide: PROTECTED_PREVIEW_MAX_WIDTH,
      });
      ...
      const previewBuffer = await applyWatermarkToImage(resizedBuffer);

      previewBufferProcessed = await sharp(previewBuffer)
        .rotate()
        .resize(PREVIEW_MAX_WIDTH, null, {
          withoutEnlargement: true,
          fit: "inside",
      ...
      previewBufferProcessed = await sharp(imageBuffer)
        .rotate()
        .resize(PREVIEW_MAX_WIDTH, null, {
          withoutEnlargement: true,
          fit: "inside",
