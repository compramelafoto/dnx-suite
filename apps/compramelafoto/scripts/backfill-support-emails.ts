...
async function main() {
  ...
  for (const ticket of ticketsToProcess) {
    ...
    try {
      const supportUrl = getSupportUrlForRole(ticket.requesterRole, ticket.id);
      ...
      if (emailResult.success) {
        console.info(`[support-email] sent ticketId=${ticket.id} to=${toEmail}`);
        await prisma.supportTicket.update({
          where: { id: ticket.id },
          data: { lastReplyEmailSentAt: new Date() },
        });
      } else {
        console.warn(`[support-email] failed ticketId=${ticket.id} to=${toEmail} error=${emailResult.error ?? "unknown"}`);
        errors++;
        continue;
      }
