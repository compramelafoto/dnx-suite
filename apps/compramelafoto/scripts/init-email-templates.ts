...
{{/buyerName}}Email: {{buyerEmail}}
{{#buyerPhone}}WhatsApp / teléfono: {{buyerPhone}}
{{/buyerPhone}}Total: {{total}}
{{#hasPrintItems}}
...
<p><strong>Cliente:</strong> {{buyerLabel}}<br>
{{#buyerName}}<strong>Nombre:</strong> {{buyerName}}<br>{{/buyerName}}
<strong>Email:</strong> {{buyerEmail}}<br>
{{#buyerPhone}}<strong>WhatsApp / teléfono:</strong> {{buyerPhone}}<br>{{/buyerPhone}}
...
<p>Saludos,<br>ComprameLaFoto</p>`,
    variables: ["photographerName", "orderId", "albumTitle", "buyerLabel", "buyerName", "buyerEmail", "buyerPhone", "total", "hasPrintItems", "pedidosUrl"],
  },
  {
    key: "order_ready",
    name: "Pedido listo",
...
<p>Saludos,<br>ComprameLaFoto</p>`,
  ...
  },
  {
    ...
    subject: "Tus fotos digitales están listas - Pedido #{{orderId}}",
    bodyText: `Hola {{customerName}},

Tus fotos digitales del pedido #{{orderId}} están listas.

Ver mis fotos: {{downloadUrl}}

Desde ahí podés descargarlas una por una o en ZIP.

Este acceso estará disponible hasta el {{expiresAt}}.

Saludos,
ComprameLaFoto`,
    bodyHtml: `<p>Hola {{customerName}},</p>
<p>Tus fotos digitales del pedido <strong>#{{orderId}}</strong> están listas.</p>
<p><a href="{{downloadUrl}}" style="display: inline-block; background: #c27b3d; color: #ffffff; padding: 10px 18px; border-radius: 6px; text-decoration: none;">Ver mis fotos</a></p>
<p><small>Desde ahí podés descargarlas una por una o en ZIP. Este acceso estará disponible hasta el {{expiresAt}}.</small></p>
<p>Saludos,<br>ComprameLaFoto</p>`,
    variables: [
      "customerName",
      "orderId",
