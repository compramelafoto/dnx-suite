/**
 * Crea o actualiza el artículo «Programa de Referidos de ComprameLaFoto» (DRAFT).
 * Ejecutar: npm run seed:blog:programa-referidos
 */

import { BlogPostStatus, PrismaClient } from "@prisma/client";
import { REFERIDOS_PROGRAMA_ARTICLE } from "@/data/blog/phase7/catalog-referidos";
...
const SLUG = "programa-referidos-compramelafoto";
const prisma = new PrismaClient();

async function main() {
  const draft = REFERIDOS_PROGRAMA_ARTICLE;
  ...
  console.log(`   Vista pública (al publicar): /blog/${SLUG}`);
}
