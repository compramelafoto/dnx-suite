...
export function buildSchoolOrganizerInviteEmail(params: {
  schoolName: string;
  inviteUrl: string;
  invitedByName?: string;
  recipientName?: string;
}) {
  const { schoolName, inviteUrl, invitedByName, recipientName } = params;
  const subject = `Invitación para administrar la escuela "${schoolName}"`;
  const html = wrapEmail(`
    <p>${greet(recipientName)}</p>
    <p>
      ${invitedByName ? `<strong>${invitedByName}</strong>` : "Un administrador"} te invitó a
      administrar la escuela <strong>${schoolName}</strong> en ComprameLaFoto.
    </p>
    <p>
      <a href="${inviteUrl}" style="display: inline-block; background: #c27b3d; color: #ffffff; padding: 10px 18px; border-radius: 6px; text-decoration: none;">
        Activar mi cuenta
    ...
    <p style="font-size: 13px; color: #6b7280;">
      Este enlace es personal y vence pronto. Si no esperabas esta invitación, podés ignorar este email.
    </p>
  `);
  return { subject, html };
}
