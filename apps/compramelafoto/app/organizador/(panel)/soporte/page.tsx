...
import { supportTicketSectionId, useSupportTicketDeepLink } from "@/hooks/useSupportTicketDeepLink";
...
export default function OrganizadorSoportePage() {
  ...
  const ticketDeepLinkParam = searchParams?.get("ticket");
  const focusedSupportTicketId = useSupportTicketDeepLink(tickets, ticketsLoading, ticketDeepLinkParam, true);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <OrganizerHeader organizer={organizer} />
      <section className="max-w-6xl mx-auto px-4 py-8 space-y-8 ds-fill-width min-w-0">
        <div className="ds-content-container max-w-3xl">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-1">Soporte</h1>
          <p className="text-gray-600 m-0 leading-relaxed">Comunicate con el equipo y creá tickets de soporte</p>
        </div>

        <Card className="p-6 sm:p-8 space-y-6 ds-card max-w-2xl w-full">
          {error && (
          ...
          <form onSubmit={handleSubmit} className="ds-form-stack">
            <div className="w-full">
              <label htmlFor="org-support-reason" className="block text-sm font-medium text-gray-900 mb-2">
                Motivo
              </label>
              <Select
                id="org-support-reason"
                className="w-full"
            ...
            <div className="w-full">
              <label htmlFor="org-support-description" className="block text-sm font-medium text-gray-900 mb-2">
                Descripción
              </label>
              <Textarea
                id="org-support-description"
                rows={5}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
              <div className="w-full min-w-0">
                <label htmlFor="org-support-name" className="block text-sm font-medium text-gray-900 mb-2">
                  Nombre
                </label>
                <Input
                  id="org-support-name"
              ...
              <div className="w-full min-w-0">
                <label htmlFor="org-support-email" className="block text-sm font-medium text-gray-900 mb-2">
                  Email
                </label>
                <Input
              ...
              <div className="w-full min-w-0 md:col-span-2">
                <label htmlFor="org-support-phone" className="block text-sm font-medium text-gray-900 mb-2">
                  Teléfono
                </label>
                <Input
            ...
            <div className="pt-1">
              <Button
                ...
                variant="primary"
                disabled={loading}
                className="w-full sm:w-auto min-w-[10rem] whitespace-nowrap"
              >
        ...
        <Card className="p-6 sm:p-8 space-y-4 ds-card w-full min-w-0">
          <h2 className="text-xl font-medium text-gray-900 m-0">Mis incidencias</h2>
          {ticketsLoading ? (
            ...
            <div className="space-y-4">
              {tickets.map((ticket: any) => (
                <div
                  key={ticket.id}
                  id={supportTicketSectionId(ticket.id)}
                  className={`rounded-lg border border-gray-200 p-4 space-y-3 scroll-mt-24 ${
                    focusedSupportTicketId === ticket.id ? "ring-2 ring-[#c27b3d]/55 shadow-sm" : ""
                  }`}
                >
                  ...
                  <p className="text-sm text-gray-700 whitespace-pre-wrap break-words ds-readable-text m-0">
                    {ticket.description}
                  </p>
                    ...
                    <div className="space-y-2 border-t border-gray-200 pt-3">
                      {ticket.messages.map((msg: any) => (
                        <div key={msg.id} className="text-sm">
                          ...
                          <div className="text-gray-700 whitespace-pre-wrap break-words">{msg.message}</div>
                        </div>
                      ))}
                    </div>
                  )}
