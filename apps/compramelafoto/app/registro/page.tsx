...
import {
  ...
  getTrainingReferralForRegistration,
  clearReferralCookies,
  type ReferralAttributionSourceClient,
...
function RegisterPageContent() {
  ...
  const isCuantoCobroSignup =
    productParam === "cuantocobro" || safeRedirect.startsWith("/cuantocobro");
  const refFromQuery = searchParams?.get("ref") ?? null;
  const sourceFromQuery = searchParams?.get("source") ?? null;
  const trainingIdFromQuery = searchParams?.get("trainingId") ?? null;
  const [refFromCookie, setRefFromCookie] = useState<string | null>(null);
  const [metaFromCookie, setMetaFromCookie] =
    useState<ReferralAttributionSourceClient | null>(null);
  ...
  const trainingReferralFinal = getTrainingReferralForRegistration(
    sourceFromQuery,
    trainingIdFromQuery,
    metaFromCookie
  );
  ...
  useEffect(() => {
    ...
    setMetaFromCookie(getReferralMetaCookie());
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (userType === "PHOTOGRAPHER") {
        ...
        redirectPath = isCuantoCobroSignup
          ? `/cuantocobro/login?registered=true${redirectQuery}`
          : `/fotografo/login?registered=true${redirectQuery}`;
      ...
      const usesReferralAttribution =
        userType === "PHOTOGRAPHER" || userType === "ORGANIZER";
      ...
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          password,
          marketingOptIn,
          ...(isCuantoCobroSignup && userType === "PHOTOGRAPHER" ? { productSource: "cuantocobro" } : {}),
          ...(usesReferralAttribution && refFinal ? { ref: refFinal } : {}),
          ...(usesReferralAttribution && trainingReferralFinal
            ? {
                sourceType: trainingReferralFinal.sourceType,
                sourceEntityId: trainingReferralFinal.sourceEntityId,
              }
            : {}),
          ...(userType === "LAB" && {
            phone: "",
      ...
      // Borrar cookies de referido y origen (capacitación) tras registro exitoso.
      clearReferralCookies();
