...
interface UserRow {
  ...
  createdAt: string;
  isBlocked: boolean;
  /** Admin: ver nombre/email/tel de compradores en pedidos pendientes de pago */
...
export default function AdminFotografosPage() {
  ...
  async function updateAllowUnpaidClientData(userId: number, value: boolean) {
    try {
      const res = await fetch(`/api/admin/users/${userId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ allowUnpaidOrderClientData: value }),
      });
      if (res.ok) {
        await loadPhotographers();
      } else {
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th
                  className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase max-w-[140px]"
                  title="Si está activado, el fotógrafo ve nombre, email y teléfono del cliente en pedidos propios aunque el pago siga pendiente. Por defecto desactivado."
                >
                  Datos cliente sin pago
                </th>
                    ...
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm">
                        <label className="inline-flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            checked={user.allowUnpaidOrderClientData === true}
                            onChange={(e) => updateAllowUnpaidClientData(user.id, e.target.checked)}
                            title="Permitir ver datos del comprador antes de que se acredite el pago"
                          />
                          <span className="text-xs text-gray-600">Habilitado</span>
                        </label>
                      </td>
                      <td className="px-4 py-3 text-sm">
                        {hasMercadoPago ? (
                      ...
                      <td className="px-4 py-3 text-sm">
                        <AdminUserCopyLinkButtons
                          ...
                          publicPageHandler={user.publicPageHandler}
