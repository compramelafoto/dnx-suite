...
function AdminPedidoDetailContent() {
  ...
  const [adminDlBusy, setAdminDlBusy] = useState(false);
  ...
  async function adminAlbumDigitalDownload(orderId: number) {
    ...
    try {
      ...
      if (data?.downloadUrl) window.open(data.downloadUrl as string, "_blank");
      else alert("No hay link de descarga disponible aún.");
    } catch (e: any) {
      alert(e?.message || "Error");
    } finally {
      setAdminDlBusy(false);
    }
  }
  ...
  if (orderSource === "ALBUM_ORDER") {
    ...
    const derived = digitalOrder.fulfillmentKind
      ? {
          kind: digitalOrder.fulfillmentKind,
          digital: digitalOrder.digitalItemsCount ?? 0,
          print: digitalOrder.printItemsCount ?? 0,
        }
      : (() => {
          const f = getAlbumOrderFulfillmentFromItems(
            digitalOrder.items.map((it) => ({ productType: it.productType }))
          );
          return { kind: f.kind, digital: f.digitalItemsCount, print: f.printItemsCount };
        })();
    const albumPaid = digitalOrder.paymentStatus === "PAID";
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <span className="text-gray-800 font-medium">{getFulfillmentKindLabel(derived.kind)}</span>
              {derived.digital > 0 && (
        ...
        <Card className="p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-3">Acciones de descarga</h2>
          <p className="text-sm text-gray-600 mb-4">
            Mismos endpoints que el panel del fotógrafo: digitales vía token; impresiones ZIP por pedido de álbum.
          </p>
          <div className="flex flex-wrap gap-2">
            {albumPaid &&
              (derived.kind === "DIGITAL" || derived.kind === "MIXED") &&
              derived.digital > 0 && (
                <Button
                  variant="primary"
                  size="sm"
                  disabled={adminDlBusy}
                  onClick={() => adminAlbumDigitalDownload(digitalOrder.id)}
                >
                  {adminDlBusy ? "Generando…" : "Descargar digitales"}
                </Button>
              )}
            {albumPaid &&
              (derived.kind === "PRINT" || derived.kind === "MIXED") &&
              derived.print > 0 && (
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={() =>
                    window.open(
                      `${typeof window !== "undefined" ? window.location.origin : ""}/api/fotografo/pedidos/${digitalOrder.id}/export-print`,
                      "_blank"
                    )
                  }
                >
                  Descargar impresiones (ZIP)
                </Button>
              )}
            {!albumPaid && (
              <p className="text-sm text-gray-500">Disponibles cuando el pago esté acreditado.</p>
            )}
          </div>
        </Card>
        ...
        <Card className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <p className="text-sm text-gray-500">Cliente pagó</p>
              <p className="text-gray-900 font-medium tabular-nums">{formatARS(digitalOrder.total)}</p>
            </div>
            <div>
        ...
        {digitalOrder.eventOrganizerSale?.active ? (
          <CollaborativeEventSaleBreakdown
            breakdown={digitalOrder.eventOrganizerSale}
            audience="admin"
          />
        ) : null}
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <p className="text-sm text-gray-500 mb-1">Fulfillment</p>
                <p className="text-gray-900">
                  {getFulfillmentKindLabel("PRINT")}{" "}
                  <span className="text-gray-600">
                    · {printOrder.items.length} ítem{printOrder.items.length === 1 ? "" : "s"} impresión
                  </span>
                </p>
              </div>
