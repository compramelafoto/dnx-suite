...
import AlbumDiagnosticsModal from "@/components/admin/AlbumDiagnosticsModal";
...
function AdminAlbumsContent() {
  ...
  const [diagAlbum, setDiagAlbum] = useState<{
    id: number;
    title: string;
    publicSlug: string | null;
  } | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {diagAlbum && (
        <AlbumDiagnosticsModal
          albumId={diagAlbum.id}
          albumTitle={diagAlbum.title}
          publicSlug={diagAlbum.publicSlug}
          open
          onClose={() => setDiagAlbum(null)}
        />
      )}
    </div>
  );
