...
export default function SoporteClient() {
  ...
  // Preserva ?ticket= y ?orderId= al normalizar.
  useEffect(() => {
    const tab = searchParams?.get("tab");
    const ticket = searchParams?.get("ticket");
    if (tab === "terminos") {
      const sp = new URLSearchParams(searchParams?.toString() ?? "");
      sp.set("tab", "politicas");
      router.replace(`/fotografo/soporte?${sp.toString()}`, { scroll: false });
      return;
    }
    const validTab = Boolean(tab && VALID_TABS.includes(tab as TabId));
    if (ticket && validTab && tab !== "incidencias") {
      const sp = new URLSearchParams(searchParams?.toString() ?? "");
      sp.set("tab", "incidencias");
      router.replace(`/fotografo/soporte?${sp.toString()}`, { scroll: false });
      return;
    }
    if (!tab || !validTab) {
      ...
      sp.set("tab", "incidencias");
      if (ticket) sp.set("ticket", ticket);
  ...
  const focusedSupportTicketId = useSupportTicketDeepLink(
    tickets,
    ticketsLoading,
    ticketDeepLinkParam,
                  ...
                  <div className="space-y-4">
                    {tickets.map((ticket) => (
                      <div
                        ...
                        className={`rounded-lg border border-[#e5e7eb] p-4 space-y-3 scroll-mt-24 ${
                      ...
                      >
                        ...
                        <p className="text-sm text-[#1a1a1a] whitespace-pre-wrap break-words">{ticket.description}</p>
                          ...
                          <div className="space-y-2 border-t border-[#e5e7eb] pt-3">
                            {ticket.messages.map((msg: any) => (
                              <div key={msg.id} className="text-sm">
                                ...
                                <div className="text-[#1a1a1a] whitespace-pre-wrap break-words">{msg.message}</div>
                              </div>
                            ))}
                          </div>
                        )}
          ...
          {activeTab === "politicas" && (
            <div className="space-y-6">
              ...
              <Card className="space-y-4">
                ...
                <Link href="/dashboard/referrals" className="text-[#c27b3d] hover:underline text-sm">Ir al Panel de Referidos →</Link>
              </Card>
            </div>
          )}
