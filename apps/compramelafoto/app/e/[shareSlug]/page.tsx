...
import EventAlbumShareBlock from "./EventAlbumShareBlock";
import { ensureEventAlbumTitles } from "@/lib/events/event-album";
import {
  eventPhotoPricingEnrollmentCopy,
  eventPhotoPricingPhotoPickerLine,
...
export default async function PublicEventPage({
  ...
  let membershipStatus: EventMemberStatus | null = null;
  ...
  const uploadsRuleText =
    "El organizador recibe 5 fotos en alta por cada fotógrafo que efectivamente suba fotos al evento.";

  const normalizedPricing = normalizedPricingFromEventDb(event);
  const photoPricingLine = eventPhotoPricingPhotoPickerLine(normalizedPricing);
  const pricingEnrollment = eventPhotoPricingEnrollmentCopy(normalizedPricing);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="w-full max-w-4xl mx-auto px-4 py-8 min-w-0 box-border">
        ...
        <p className="text-sm text-gray-500 mb-4">
          {activePhotographersCount} fotógrafo(s) inscrito(s)
        ...
        <EventJoinBlock
          ...
          activePhotographersCount={activePhotographersCount}
          isPrivateOrInviteOnly={isPrivateOrInviteOnly}
          isInvited={isInvited}
          termsText={resolveEventPhotographerTerms(event)}
          uploadsRuleText={uploadsRuleText}
          photoPricingLine={photoPricingLine}
          pricingEnrollment={pricingEnrollment}
        />
        <EventAlbumShareBlock
          shareUrl={`${baseUrl}/e/${shareSlug}`}
          title="Link de invitación para fotógrafos"
          description="Usá este link para invitar fotógrafos al evento y que puedan inscribirse."
          qrLabel="Escaneá o descargá este QR para compartir la invitación."
        />
        <EventAlbumShareBlock
          shareUrl={`${baseUrl}/g/${shareSlug}`}
          title="Link público de la galería"
          description="Este es el link único para compartir la galería del evento con clientes."
          qrLabel="Escaneá o descargá este QR para compartir la galería."
        />
        <EventGallery shareSlug={shareSlug} />
      </main>
    </div>
  );
}
