...
export default function DatosPage() {
  ...
  useEffect(() => {
    ...
    if (savedClient) {
      try {
        ...
        // Prellenar datos si el cliente está autenticado
  ...
  useEffect(() => {
    async function loadPricing() {
      try {
        const storedPhotographerId = Number(
  ...
  const summaryBySizeAndFinish = useMemo(() => {
    const map = new Map<string, number>(); // key: "size-finish", value: cantidad total
    for (const it of items) {
    ...
    // Ordenar por tamaño y luego por acabado
    entries.sort((a, b) => {
  ...
  async function handleSubmit() {
    ...
    const emailError = getCheckoutEmailValidationError(customerEmail);
    if (emailError) {
      setError(emailError);
      return;
    }
    ...
    try {
      ...
      const res = await fetch("/api/print-orders", {
        ...
        headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            customerEmail,
            customerPhone,
            clientId: client?.id || null, // Vincular con cliente autenticado si existe
            pickupBy: pickupBy, // Opción seleccionada por el cliente
            photographerId,
            items: items.map((it) => ({
              fileKey: it.fileKey,
              originalName: it.originalName,
              size: it.size,
              acabado: it.finish, // Backend espera 'acabado'
      ...
      try {
        ...
        if (!prefRes.ok) {
          throw new Error(prefData?.error || "Error creando preferencia de pago");
        }
        ...
        // Si falla la creación de preferencia, igual redirigir a confirmación
        // pero mostrar un mensaje de error
        setError(`Pedido creado (#${data.id}), pero error al generar link de pago: ${mpError.message}`);
        sessionStorage.removeItem("uploadedPhotos");
        sessionStorage.removeItem("orderItems");
        router.push("/imprimir/confirmacion");
        return;
      }

      // Si no hay initPoint, redirigir a confirmación
      sessionStorage.removeItem("uploadedPhotos");
      sessionStorage.removeItem("orderItems");
      router.push("/imprimir/confirmacion");
    } catch (err: any) {
      ...
      <div className="container-custom">
        <div className="max-w-5xl mx-auto space-y-8" style={{ wordBreak: "normal", overflowWrap: "normal" }}>
          ...
          <Card className="space-y-6">
                ...
                <>
                  ...
                  {!customerEmail.trim() ? (
                    <p className="text-xs text-[#6b7280] mt-1">
                      {EMAIL_EMPTY_PLACEHOLDER_COPY.print}
                    </p>
                  ) : null}
                  <EmailConfirmationHint
                    email={customerEmail}
                    variant="print"
                    onApplySuggestion={setCustomerEmail}
                    className="mt-1"
                  />
                </>
              )}
            ...
            <div>
              <label className="block text-sm font-medium text-[#1a1a1a] mb-3">
                ¿Dónde retirás el pedido? *
              </label>
              <div className="space-y-3">
                <label className="flex items-start gap-3 p-3 border border-[#e5e7eb] rounded-lg cursor-pointer hover:bg-[#f8f9fa] transition-colors">
                  <input
                    type="radio"
                    name="pickupBy"
                    value="CLIENT"
                  ...
                  <div className="flex-1">
                    ...
                    <span className="text-xs text-[#6b7280]">
                      Retirás directamente en el laboratorio cuando esté listo
                    </span>
                  </div>
                </label>
                <label className="flex items-start gap-3 p-3 border border-[#e5e7eb] rounded-lg cursor-pointer hover:bg-[#f8f9fa] transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    <span className="text-xs text-[#6b7280]">
                      El fotógrafo retira el pedido del laboratorio y lo entrega en su estudio
                    </span>
                  </div>
                </label>
              </div>
            ...
            {!client && (
              <div className="pt-4 border-t border-[#e5e7eb]">
                ...
                <Link href="/registro" className="text-sm text-[#c27b3d] hover:underline">
          ...
          <div className="flex justify-between pt-6">
            ...
            <Button variant="primary" onClick={handleSubmit} disabled={loading || !pricingLoaded}>
