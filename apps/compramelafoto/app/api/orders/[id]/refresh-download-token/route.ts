...
export async function POST(
  ...
  try {
    ...
    const baseUrl =
      ...
      "";
    const downloadUrl = buildZipDownloadApiUrl(token, baseUrl);
    const rolloutActive = logDownloadCenterRolloutDecision({
      orderId: order.id,
      orderCreatedAt: order.createdAt,
      context: "refresh_download_token",
    });
    const downloadCenterUrl = rolloutActive
      ? buildDownloadCenterUrl(token, baseUrl)
      : null;

    return NextResponse.json({
      downloadUrl,
      downloadCenterUrl,
      downloadCenterRolloutActive: rolloutActive,
