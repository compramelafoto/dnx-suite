...
export async function GET(req: NextRequest) {
  ...
  const recovered = await recoverStaleDesignJobs("preview");
  if (recovered.reencolados > 0 || recovered.descartados > 0) {
    console.info("[school_design_preview_job] stale_jobs_recovered", recovered);
  }
  ...
  for (const job of jobs) {
    ...
    await clearTerminalDesignJobs("preview", job.designRevisionId, job.id);
