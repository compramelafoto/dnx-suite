...
async function ensureAlbumAccess(albumId: number) {
  const album = await prisma.album.findUnique({
    where: { id: albumId },
    select: { id: true, userId: true, isPublic: true, isHidden: true, isTest: true },
  });
  if (!album) return { ok: false, album: null };

  const authUser = await getAuthUser();
  const testDeny = await denyIfTestAlbumNotOwnerPreview(album, authUser);
  if (testDeny) {
    return { ok: false, album: null, testDenyResponse: testDeny };
  }
...
export async function POST(
  ...
  if ("testDenyResponse" in access && access.testDenyResponse) {
    return access.testDenyResponse;
  }
  if (!access.ok) {
