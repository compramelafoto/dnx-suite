...
export async function POST(
  ...
  // Solo fotos ya analizadas sin rostro (DONE). Las pendientes de análisis no se exponen.
  const noFacePhotoIds = albumPhotos
    .filter((p) => p.analysisStatus === "DONE" && !photoIdsWithFace.has(p.id))
    .map((p) => p.id);
  const pendingAnalysisCount = albumPhotos.filter(
  ...
  if (pendingAnalysisCount > 0 && photosMatchedCount === 0 && noFacePhotoIds.length === 0) {
    await prisma.hiddenAlbumAttempt.update({
      where: { id: attempt.id },
      data: {
        result: HiddenAlbumAttemptResult.ERROR,
        errorCode: "ANALYSIS_PENDING",
        errorMessage: "Fotos en procesamiento",
        facesInSelfieCount: 1,
        durationMs: Date.now() - startTotal,
      },
    });
    return NextResponse.json(
      {
        error:
          "Las fotos del álbum se están procesando. Esperá unos minutos y volvé a intentar el reconocimiento facial.",
      },
      { status: 503 }
    );
  }
