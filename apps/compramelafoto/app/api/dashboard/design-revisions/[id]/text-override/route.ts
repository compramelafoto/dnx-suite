...
const BodySchema = z.object({
  textFieldId: z.string().trim().min(1),
...
export async function POST(
  ...
  const result = parsed.data.clear
    ? clearTextOverrideInData(data, parsed.data.textFieldId)
    : updateTextOverrideInData(data, parsed.data.textFieldId, parsed.data.overrideValue ?? null);
  if (!result.ok) {
    console.warn("[school_design_editor] invalid_editor_action", {
      designProjectId: ctx.id,
      revisionId,
      action: "text_override",
    });
  ...
  console.info(
    parsed.data.clear ? "[school_design_editor] text_override_cleared" : "[school_design_editor] text_override_updated",
    {
      designProjectId: ctx.id,
      revisionId,
      textFieldId: parsed.data.textFieldId,
    }
  );
