...
import { deletePhotoPublicPreviewAssets } from "@/lib/photo-r2-cleanup";
...
export async function PATCH(
  ...
  try {
    ...
    // Si se aprueba, marcar la foto como removida y quitar previews de R2
    if (action === "APPROVE") {
      const photoIsRemoved = request.photo?.isRemoved ?? false;
      if (!photoIsRemoved) {
        const photoForCleanup = await prisma.photo.findUnique({
        ...
        if (photoForCleanup) {
          await deletePhotoPublicPreviewAssets(photoForCleanup);
        }
