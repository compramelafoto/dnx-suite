...
export async function POST() {
  try {
    ...
    const settings = await prisma.cameraConnectionSettings.update({
      ...
      data: { enabled: false, paused: false },
    ...
    console.error("POST /api/dashboard/camera-connection/disable", err);
    return cameraConnectionError("Error al desactivar Conexión de Cámara.");
  }
}
