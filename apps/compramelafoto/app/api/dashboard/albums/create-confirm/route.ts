  ...
  try {
    ...
    if (selectedEventId && !createNew) {
      ...
      const event = await prisma.event.findUnique({
        where: { id: eventId },
        select: { id: true, title: true, latitude: true, longitude: true, startsAt: true, endsAt: true, type: true, city: true },
      });
      ...
      const albumData: any = {
        ...
        isPublic: eventAlbumVisibility.isPublic,
        isHidden: eventAlbumVisibility.isHidden,
        ...
        enablePrintedPhotos: albumFields.enablePrintedPhotos === true,
        enableDigitalPhotos: albumFields.enableDigitalPhotos === true,
    ...
    const enablePrinted = albumFields.enablePrintedPhotos === true;
    if (enablePrinted) {
    ...
    if (
      albumFields.eventDate != null ||
      albumFields.eventStartTime != null ||
      albumFields.eventEndTime != null
    ) {
      scheduleInput = {
        eventDate: albumFields.eventDate != null ? String(albumFields.eventDate) : null,
        eventStartTime:
          albumFields.eventStartTime != null ? String(albumFields.eventStartTime) : null,
        eventEndTime:
          albumFields.eventEndTime != null ? String(albumFields.eventEndTime) : null,
      };
    } else if (albumFields.startsAt) {
      const fromDb = buildAlbumEventScheduleFromDb({
        startsAt: new Date(albumFields.startsAt),
      });
      scheduleInput = {
        eventDate: fromDb.eventDate || null,
        eventStartTime: fromDb.eventStartTime || null,
        eventEndTime: fromDb.eventEndTime || null,
      };
    } else {
      scheduleInput = { eventDate: null, eventStartTime: null, eventEndTime: null };
    }
    const scheduleValidation = validateAlbumEventSchedule(scheduleInput);
    if (!scheduleValidation.ok) {
      return NextResponse.json({ error: scheduleValidation.error }, { status: 400 });
    }
    const eventSchedule = parseAlbumEventScheduleInput(scheduleInput);
    ...
    const albumData: any = {
      ...
      longitude: lng,
      geohash,
      startsAt: eventSchedule.startsAt,
      endsAt: eventSchedule.endsAt,
