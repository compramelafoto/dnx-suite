...
function mergeAlbumEventScheduleInput(
  body: {
    eventDate?: unknown;
    eventStartTime?: unknown;
    eventEndTime?: unknown;
  },
  existing: { eventDate: Date | null; startsAt: Date | null; endsAt: Date | null }
): AlbumEventScheduleInput {
  const current = buildAlbumEventScheduleFromDb(existing);
  return {
    eventDate:
    ...
    eventStartTime:
      body.eventStartTime !== undefined
        ? body.eventStartTime != null && body.eventStartTime !== ""
          ? String(body.eventStartTime)
          : null
        : current.eventStartTime || null,
    eventEndTime:
      body.eventEndTime !== undefined
        ? body.eventEndTime != null && body.eventEndTime !== ""
          ? String(body.eventEndTime)
          : null
        : current.eventEndTime || null,
  };
}
...
export async function GET(
  ...
  try {
    ...
    const includePhotos = albumDashboardGetIncludePhotos(req.nextUrl.searchParams);

    const albumRelationInclude = {
          ...
          if (useMinimalPhotos) {
            ...
            if (album) {
              ...
              if (includePhotos) {
                ...
                (album as any).photos = (photos as any[]).map((p: any) => ({
                  ...p,
                  sellDigital: true,
                  sellPrint: true,
                }));
    ...
    let dashboardPhotos: Array<Record<string, unknown>> = [];
    if (includePhotos) {
      const photos = (album as any).photos as Array<{
        id: number;
        userId?: number | null;
        previewUrl?: string;
        originalKey?: string;
    ...
    delete (album as { photos?: unknown }).photos;
    ...
    let eventShareSlug: string | null = null;
    let eventCollaborativePhotoPricing: CollaborativeEventPricingLockSnapshot | null = null;
    let eventFolders: Array<{
      id: number;
      name: string;
    ...
    const eventIdRaw = (album as any)?.eventId;
    const hasEventAlbum =
      typeof eventIdRaw === "number" &&
      Number.isFinite(eventIdRaw) &&
      eventIdRaw > 0;

    let photoStats: AlbumPhotoStats;
    try {
      photoStats = await loadAlbumPhotoStats(
        prisma,
        albumId,
        album.userId as number,
        user.id,
        hasEventAlbum ? "event" : "album"
      );
    } catch {
      photoStats = {
        total: 0,
    ...
    let eventFolderPhotoCounts: Record<number, number> = {};
    if (hasEventAlbum) {
      try {
        eventFolderPhotoCounts = await loadEventFolderPhotoCounts(prisma, albumId);
      } catch {
        eventFolderPhotoCounts = {};
      }
    }
    ...
    if (hasEventAlbum) {
      try {
        ...
        eventShareSlug = eventRow?.shareSlug ?? null;
        eventCollaborativePhotoPricing = eventRow
          ? collaborativeEventPricingFromPricingRow({
              photoPricingMode: eventRow.photoPricingMode,
              fixedPhotoPrice: eventRow.fixedPhotoPrice,
              minimumPhotoPrice: eventRow.minimumPhotoPrice,
            })
          : null;
      } catch {}

      try {
        const isOrganizer = await isEventOrganizerUser(user.id, eventIdRaw);
            ...
            ...(isOrganizer
              ? {
                  OR: [
                    ...
                    {
                      folderScope: EventFolderScope.PHOTOGRAPHER,
        ...
        eventFolders = rows.map((r) => ({
          id: r.id,
          name: r.name,
          parentId: r.parentId ?? null,
          folderScope: r.folderScope,
          sortOrder: r.sortOrder,
          listedInPublicGallery: r.listedInPublicGallery,
          isMine:
            r.folderScope === EventFolderScope.PHOTOGRAPHER &&
            r.ownerPhotographerId === user.id,
          photoCount: eventFolderPhotoCounts[r.id] ?? 0,
        }));
      } catch {
        eventFolders = [];
      }
    } else if (album.userId === user.id) {
      try {
    ...
    return NextResponse.json({
      ...
      eventSchedule: buildAlbumEventScheduleFromDb({
        ...
        startsAt: (album.startsAt as Date | null | undefined) ?? null,
        endsAt: (album.endsAt as Date | null | undefined) ?? null,
      }),
      photographerHandler,
      eventShareSlug,
      eventFolders,
...
export async function PATCH(
  ...
  try {
    ...
    const collaborativeEventPricing =
      existingAlbum.eventId != null
        ? await loadCollaborativeEventPricingSnapshot(existingAlbum.eventId)
        : null;
    if (
      collaborativeEventPricing?.locksPhotographerDigitalPricing &&
      digitalPhotoPriceCents !== undefined
    ) {
      return NextResponse.json(
    ...
    const linkedToEvent = existingAlbum.eventId != null;
    ...
    if (title !== undefined && !linkedToEvent) {
    ...
    if (location !== undefined && !linkedToEvent) {
    ...
    if (
      ...
      const scheduleInput = mergeAlbumEventScheduleInput(
        { eventDate, eventStartTime, eventEndTime },
        {
          eventDate: existingAlbum.eventDate,
          startsAt: existingAlbum.startsAt,
          endsAt: existingAlbum.endsAt,
        }
      );
      ...
      const schedule = parseAlbumEventScheduleInput(scheduleInput);
      updateData.eventDate = schedule.eventDate;
      updateData.startsAt = schedule.startsAt;
      updateData.endsAt = schedule.endsAt;
    }
    ...
    if (selectedLabId !== undefined) {
      if (selectedLabId === null || selectedLabId === "") {
        ...
        const parsedLabId = Number(selectedLabId);
        if (!Number.isFinite(parsedLabId) || parsedLabId <= 0) {
          return NextResponse.json({ error: "selectedLabId inválido" }, { status: 400 });
        }
        const labExists = await prisma.lab.findUnique({ where: { id: parsedLabId } });
        if (!labExists) {
          return NextResponse.json({ error: "Laboratorio no encontrado" }, { status: 400 });
        }
        updateData.selectedLabId = parsedLabId;
      }
    }

    if (pickupBy !== undefined) {
      updateData.pickupBy = pickupBy === "PHOTOGRAPHER" ? "PHOTOGRAPHER" : "CLIENT";
    }

    if (!collaborativeEventPricing?.locksPhotographerDigitalPricing) {
      if (digitalDiscount5Plus !== undefined) {
        updateData.digitalDiscount5Plus =
          digitalDiscount5Plus !== null && digitalDiscount5Plus !== ""
            ? parseFloat(String(digitalDiscount5Plus))
            : null;
      }
      if (digitalDiscount10Plus !== undefined) {
        updateData.digitalDiscount10Plus =
          digitalDiscount10Plus !== null && digitalDiscount10Plus !== ""
            ? parseFloat(String(digitalDiscount10Plus))
            : null;
      }
      if (digitalDiscount20Plus !== undefined) {
        updateData.digitalDiscount20Plus =
          digitalDiscount20Plus !== null && digitalDiscount20Plus !== ""
            ? parseFloat(String(digitalDiscount20Plus))
            : null;
      }
    }

    if (enableDigitalPhotos !== undefined) {
      updateData.enableDigitalPhotos = Boolean(enableDigitalPhotos);
    }
    if (enablePrintedPhotos !== undefined) {
      updateData.enablePrintedPhotos = Boolean(enablePrintedPhotos);
    }

    if (includeDigitalWithPrint !== undefined) {
      (updateData as { includeDigitalWithPrint?: boolean }).includeDigitalWithPrint =
        Boolean(includeDigitalWithPrint);
      if (!includeDigitalWithPrint) {
        (updateData as { digitalWithPrintDiscountPercent?: number }).digitalWithPrintDiscountPercent = 0;
      }
    }
    if (digitalWithPrintDiscountPercent !== undefined && includeDigitalWithPrint !== false) {
      const parsed = Number(digitalWithPrintDiscountPercent);
      (updateData as { digitalWithPrintDiscountPercent?: number }).digitalWithPrintDiscountPercent =
        Number.isFinite(parsed) ? Math.min(100, Math.max(0, parsed)) : 0;
    }

    const needsTermsAcceptance =
      !existingAlbum.termsAcceptedAt || existingAlbum.termsVersion !== TERMS_VERSION;
    if (needsTermsAcceptance && termsAccepted === true) {
      (updateData as { termsAcceptedAt?: Date; termsVersion?: string }).termsAcceptedAt =
        new Date();
      (updateData as { termsVersion?: string }).termsVersion = TERMS_VERSION;
    }

    if (showComingSoonMessage !== undefined) {
      (updateData as { showComingSoonMessage?: boolean }).showComingSoonMessage =
    ...
    if (mode !== undefined) {
      const validModes: AlbumMode[] = ["SIMPLE", "EVENT", "SCHOOL", "COLLABORATIVE"];
      if (typeof mode !== "string" || !validModes.includes(mode as AlbumMode)) {
        return NextResponse.json({ error: "mode inválido" }, { status: 400 });
      }
      (updateData as any).mode = mode as AlbumMode;
    }

    if (studentIdentificationMode !== undefined) {
      const allowed: StudentIdentificationMode[] = [
        "NONE",
        "MANUAL",
        "ROSTER_OPTIONAL",
        "ROSTER_REQUIRED",
      ];
      if (studentIdentificationMode === null || studentIdentificationMode === "") {
        (updateData as any).studentIdentificationMode = null;
      } else if (
        typeof studentIdentificationMode === "string" &&
        allowed.includes(studentIdentificationMode as StudentIdentificationMode)
      ) {
        (updateData as any).studentIdentificationMode =
          studentIdentificationMode as StudentIdentificationMode;
      } else {
        return NextResponse.json(
          { error: "studentIdentificationMode inválido (NONE, MANUAL, ROSTER_OPTIONAL, ROSTER_REQUIRED o vacío)" },
          { status: 400 }
        );
      }
    }
    if (allowManualStudentFallback !== undefined) {
      (updateData as any).allowManualStudentFallback = Boolean(allowManualStudentFallback);
    }
    if (organizerCommissionEnabled !== undefined) {
      (updateData as any).organizerCommissionEnabled = Boolean(organizerCommissionEnabled);
    }
    if (organizerCommissionPercentage !== undefined) {
      if (
        organizerCommissionPercentage === null ||
        organizerCommissionPercentage === ""
      ) {
        (updateData as any).organizerCommissionPercentage = null;
      } else {
        const value = Number(organizerCommissionPercentage);
        ...
        (updateData as any).organizerCommissionPercentage = value;
      }
    }
    if (organizerCommissionAppliesTo !== undefined) {
      const allowed: OrganizerCommissionAppliesTo[] = ["PREVENTA", "POST_EVENT", "EXTRAS"];
      if (!Array.isArray(organizerCommissionAppliesTo)) {
        return NextResponse.json(
          { error: "organizerCommissionAppliesTo debe ser un array" },
          { status: 400 }
        );
      }
      const parsed = organizerCommissionAppliesTo
        .map((entry) => String(entry))
        .filter((entry): entry is OrganizerCommissionAppliesTo =>
          allowed.includes(entry as OrganizerCommissionAppliesTo)
        );
      (updateData as any).organizerCommissionAppliesTo = parsed;
    }
    if (isTest !== undefined) {
      (updateData as any).isTest = Boolean(isTest);
    }
    if (albumPackPayEnabled !== undefined) {
      (updateData as any).albumPackPayEnabled = Boolean(albumPackPayEnabled);
    }
    ...
    try {
      ...
      const errorMsg = String((updateError as Error)?.message ?? updateError ?? "");
      const missingHiddenColumns =
        errorMsg.includes("hiddenPhotosEnabled") || errorMsg.includes("hiddenSelfieRetentionDays");
      if (!missingHiddenColumns) {
        throw updateError;
      }
      console.warn(
        "PATCH: columnas hiddenPhotosEnabled/hiddenSelfieRetentionDays no existen. Ejecutá: npx prisma migrate deploy"
      );
      hiddenColumnsWarning =
        'La opción "Fotos ocultas hasta selfie" no se guardó. En el servidor ejecutá: npx prisma migrate deploy';
      const fallbackData = { ...updateData };
      delete fallbackData.hiddenPhotosEnabled;
      delete fallbackData.hiddenSelfieRetentionDays;
      if (Object.keys(fallbackData).length === 0) {
        return NextResponse.json({ error: hiddenColumnsWarning }, { status: 503 });
      }
      album = await prisma.album.update({
        where: { id: albumId },
        data: fallbackData,
    ...
    const photosCount = await countActiveAlbumPhotosForAlbum(prisma, albumId);
    ...
    if (album.hiddenPhotosEnabled) {
      try {
        const queued = await ensureAlbumPhotosQueuedForHiddenMode(albumId);
        if (queued > 0) {
          payload._hiddenAnalysisQueued = queued;
        }
      } catch (queueErr) {
...
export async function PUT(
  ...
  try {
    ...
    const collaborativeEventPricingPut =
      existingAlbum.eventId != null
        ? await loadCollaborativeEventPricingSnapshot(existingAlbum.eventId)
        : null;
    const locksOfficialDigital =
      collaborativeEventPricingPut?.locksPhotographerDigitalPricing === true;

    // Parsear precio digital (mínimo definido por configuración)
    let parsedDigital: number | null = null;
    if (!locksOfficialDigital) {
    ...
    if (linkedToEvent) {
      ...
      persistedSchedule = parseAlbumEventScheduleInput(scheduleInput);
    }

    const updateData: any = {
      title: linkedToEvent ? existingAlbum.title : title.trim(),
      location: linkedToEvent ? existingAlbum.location : location?.trim() || null,
      eventDate: persistedSchedule.eventDate,
      startsAt: persistedSchedule.startsAt,
      endsAt: persistedSchedule.endsAt,
      albumProfitMarginPercent: parsedMargin,
      printPricingSource: parsedPrintPricingSource,
      digitalPhotoPriceCents: locksOfficialDigital
        ? existingAlbum.digitalPhotoPriceCents
        : parsedDigital !== null
          ? parsedDigital
    ...
    if (locksOfficialDigital) {
      updateData.digitalDiscount5Plus = existingAlbum.digitalDiscount5Plus;
      updateData.digitalDiscount10Plus = existingAlbum.digitalDiscount10Plus;
      updateData.digitalDiscount20Plus = existingAlbum.digitalDiscount20Plus;
    } else {
      if (digitalDiscount5Plus !== undefined) {
        updateData.digitalDiscount5Plus =
          digitalDiscount5Plus !== null && digitalDiscount5Plus !== ""
    ...
    try {
      ...
      if (missingHiddenColumns || errorMsg.includes("Unknown argument") || 
        ...
        console.warn("Schema no actualizado, actualizando álbum sin campos nuevos. Ejecuta: npx prisma db push && npx prisma generate");
        ...
        const allowedKeys = [
          "title", "location", "eventDate", "startsAt", "endsAt", "albumProfitMarginPercent", "printPricingSource",
        ...
        if (safeUpdateData.startsAt !== undefined) setParts.push(Prisma.sql`"startsAt" = ${safeUpdateData.startsAt}`);
        if (safeUpdateData.endsAt !== undefined) setParts.push(Prisma.sql`"endsAt" = ${safeUpdateData.endsAt}`);
        ...
        const photoCountRows = await prisma.$queryRaw<Array<{ count: number }>>(
          Prisma.sql`SELECT COUNT(*)::int AS count FROM "Photo" WHERE "albumId" = ${albumId} AND "isRemoved" = false`
        );
        const row = rows[0];
        if (!row) {
        ...
        album = row as any;
        const photosCount = photoCountRows[0]?.count ?? 0;
        const payload: Record<string, unknown> = {
          ...album,
          photosCount,
        };
...
export async function DELETE(
  ...
  try {
    ...
    const { deletePhotoR2Assets } = await import("@/lib/photo-r2-cleanup");

    const deletePhotoFiles = async (photo: {
      originalKey: string;
      ...
      try {
        ...
        console.error(`Error eliminando archivos de R2 para foto:`, error);
      }
    };
