...
export async function POST(
  ...
  try {
    ...
    const slotIdA = Number(body.slotIdA);
    const slotIdB = Number(body.slotIdB);

    if (!Number.isInteger(albumId) || !Number.isInteger(designProjectId) || !Number.isInteger(slotIdA) || !Number.isInteger(slotIdB)) {
    ...
    console.error("[school_design_editor] swap-slots", e);
