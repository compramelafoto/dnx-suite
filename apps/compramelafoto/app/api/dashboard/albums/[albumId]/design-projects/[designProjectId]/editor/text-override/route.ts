...
import {
  clearTextOverrideInData,
  parseRevisionDataJson,
  updateTextOverrideInData,
} from "@/lib/school-design/editor-data";
...
export async function POST(
  ...
  try {
    ...
    const textId = body.textId != null ? String(body.textId) : "";
    const value = body.value != null ? String(body.value) : "";
    const clear = Boolean(body.clear);

    if (!Number.isInteger(albumId) || !Number.isInteger(designProjectId) || !textId) {
    ...
    if (!data) {
      return NextResponse.json({ error: "dataJson inválido" }, { status: 400 });
    }

    const next = clear ? clearTextOverrideInData(data, textId) : updateTextOverrideInData(data, textId, value);
    ...
    console.error("[school_design_editor] text-override", e);
