...
export async function PATCH(
  request: NextRequest,
  { params }: { params: { albumId: string; photoId: string } | Promise<{ albumId: string; photoId: string }> }
) {
  try {
    ...
    const { albumId: albumIdStr, photoId: photoIdParam } = await Promise.resolve(params);
