...
import { readFromR2, getSignedUrlForFile, fileExistsInR2 } from "@/lib/r2-client";
import { createZipJob, getZipExpiresAt } from "@/lib/zip-job-queue";
import { getAuthUser } from "@/lib/auth";
import { normalizeEmail } from "@/lib/order-claims";
import { readDigitalDeliveryBuffer, getR2KeyFromPhotoKey } from "@/lib/digital-delivery-r2-key";
...
async function streamDigitalPhotoDownload(photo: {
  id: number;
  originalKey: string;
  previewUrl: string | null;
}) {
  const { buffer, r2Key } = await readDigitalDeliveryBuffer({
    id: photo.id,
    originalKey: photo.originalKey,
    previewUrl: photo.previewUrl,
  });
  const rawName = r2Key.split("/").pop() || `foto-${photo.id}.jpg`;
  const filename = safeFilename(rawName, `foto-${photo.id}`);
  ...
  try {
    if (downloadToken.type === "CLIENT_DIGITAL") {
      ...
      if (downloadToken.photoId) {
        ...
        try {
          return await streamDigitalPhotoDownload(item.photo);
        } catch (error: unknown) {
        ...
        // Descarga completa del pedido (ZIP) — solo ítems digitales
        const digitalItems = order.items.filter((item) => item.productType === "DIGITAL");
        const photos = digitalItems.map((item) => item.photo);
        ...
        if (photos.length === 1) {
          ...
          try {
            return await streamDigitalPhotoDownload(photo);
          } catch (error: unknown) {
            console.error(LOG, "R2 read failed", {
              photoId: photo.id,
              originalKey: photo.originalKey,
        ...
        if (existingJob?.status === "COMPLETED") {
          ...
          if (r2Key && (await fileExistsInR2(r2Key))) {
            try {
            ...
            console.warn(LOG, "ZIP COMPLETED pero archivo ausente en R2; regenerando", {
              orderId: order.id,
            ...
            await prisma.zipGenerationJob.update({
              ...
              data: { status: "FAILED", error: "Archivo ZIP ausente en R2" },
            });
          }
        }
