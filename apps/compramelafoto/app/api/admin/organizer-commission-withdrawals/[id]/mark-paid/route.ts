...
export async function POST(
  ...
  try {
    ...
    console.error("POST .../mark-paid ERROR >>>", err);
    return NextResponse.json(
      { error: "Error al marcar como pagado", detail: String((err as Error)?.message ?? err) },
      { status: 500 }
    );
  }
}
