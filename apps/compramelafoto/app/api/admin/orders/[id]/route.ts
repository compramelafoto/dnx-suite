...
export async function GET(
  ...
  try {
    ...
    const fulfillment = getAlbumOrderFulfillmentFromItems(order.items);

    const eventOrganizerCommissionRow = await prisma.eventOrganizerCommission.findUnique({
      where: { orderId: order.id },
      include: { event: { select: { id: true, title: true } } },
    });
    const eventOrganizerSale = eventOrganizerCommissionRow
      ? mapEventOrganizerCommissionRowToBreakdown(eventOrganizerCommissionRow)
      : null;
    ...
    return NextResponse.json({
      ...
      digitalItemsCount: fulfillment.digitalItemsCount,
      printItemsCount: fulfillment.printItemsCount,
      ...
      referral,
      eventOrganizerSale,
      platformCommissionCents: order.platformCommissionCents ?? null,
    });
  } catch (err: any) {
