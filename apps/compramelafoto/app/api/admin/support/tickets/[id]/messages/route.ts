...
export async function POST(
  ...
  try {
    ...
    const supportUrl = getSupportUrlForRole(ticket.requesterRole, ticketId);
    if (toEmail) {
      try {
        ...
        if (emailResult.success) {
          console.info(`[support-email] sent ticketId=${ticketId} to=${toEmail}`);
          await prisma.supportTicket.update({
            where: { id: ticketId },
          ...
          console.warn(
            `[support-email] failed ticketId=${ticketId} to=${toEmail} error=${emailResult.error ?? "unknown"}`
          );
        }
      } catch (emailErr: unknown) {
        console.warn(
          `[support-email] failed ticketId=${ticketId} to=${toEmail} error=${String((emailErr as Error)?.message ?? emailErr)}`
        );
      }
    }
