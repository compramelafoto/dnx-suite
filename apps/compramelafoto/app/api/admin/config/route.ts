...
export async function GET(req: NextRequest) {
  ...
  console.log("[auth_timing] admin_config_start", { requestId, ts: startedAt });
  try {
    // Verificar autenticación y rol ADMIN
    const requireAuthStart = Date.now();
    const { error, user } = await requireAuth([Role.ADMIN]);
    console.log("[auth_timing] admin_config_stage", {
      requestId,
      stage: "require_auth_done",
      durationMs: Date.now() - requireAuthStart,
    ...
    if (error || !user) {
      ...
      if (error?.includes("rol ADMIN") || error?.includes("No autorizado")) {
        console.log("[auth_timing] admin_config_done", {
          requestId,
    ...
    console.log("[auth_timing] admin_config_stage", {
      requestId,
      stage: "config_query_done",
      durationMs: Date.now() - configQueryStart,
    ...
    console.error("[auth_timing] admin_config_error", {
      requestId,
      durationMs: Date.now() - startedAt,
  ...
  try {
    ...
    const config = await prisma.appConfig.upsert({
      ...
      create: {
        ...
        downloadLinkDays: updateData.downloadLinkDays ?? 15,
