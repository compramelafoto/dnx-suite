...
export async function GET(req: NextRequest) {
  try {
    ...
    const allPaidAlbumOrderIds: number[] = [];
    albums.forEach((album) => {
      album.orders.forEach((order) => {
        if (order.status === "PAID") allPaidAlbumOrderIds.push(order.id);
      });
    });
    const organizerNetByOrderId = new Map<number, number>();
    if (allPaidAlbumOrderIds.length > 0) {
      const commissionRows = await prisma.eventOrganizerCommission.findMany({
        where: { orderId: { in: allPaidAlbumOrderIds } },
        include: { event: { select: { title: true } } },
      });
      for (const row of commissionRows) {
        const b = mapEventOrganizerCommissionRowToBreakdown(row);
        organizerNetByOrderId.set(row.orderId, b.photographerNetAmount);
      }
    }

    const amountForPhotographerReport = (orderId: number, orderTotal: number) => {
      const net = organizerNetByOrderId.get(orderId);
      if (net != null) return net;
      return orderTotal;
    };

    // Calcular estadísticas de ventas (neto fotógrafo cuando hay comisión de evento)
    let totalSalesARS = 0;
    let totalClientPaidARS = 0;
    let digitalSalesARS = 0;
    let printSalesARS = 0;
    let eventOrganizerCommissionSalesCount = 0;
    ...
    albums.forEach((album) => {
      album.orders.forEach((order) => {
        if (order.status === "PAID") {
          ...
          const reported = amountForPhotographerReport(order.id, orderTotal);
          if (organizerNetByOrderId.has(order.id)) eventOrganizerCommissionSalesCount += 1;
          totalSalesARS += reported;
          totalClientPaidARS += orderTotal;
          ...
          if (hasDigital) {
            digitalSalesARS += reported;
          }
          if (hasPrint) {
            printSalesARS += reported;
          }
          ...
          if (existing) {
            existing.total += reported;
            existing.clientPaid += orderTotal;
            if (hasDigital) existing.digital += reported;
            if (hasPrint) existing.print += reported;
          } else {
            salesHistory.push({
              date: dateKey,
              digital: hasDigital ? reported : 0,
              print: hasPrint ? reported : 0,
              total: reported,
              clientPaid: orderTotal,
            });
          }
        }
      });
    ...
    return NextResponse.json({
      stats: {
        totalSalesARS,
        totalClientPaidARS,
        eventOrganizerCommissionSalesCount,
        digitalSalesARS,
        printSalesARS,
