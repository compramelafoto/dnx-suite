...
function friendlyMessage(reason: string): string {
  ...
  if (reason === "invalid_status") {
    return "Solo podés marcar como en escuela cuando el ítem está en impresión.";
  }
  return "No se pudo marcar el ítem como entregado en la escuela.";
}

/**
 * POST: pasar un ítem escolar de PHYSICAL_IN_PROGRESS a AT_SCHOOL (fotógrafo).
 */
export async function POST(_req: Request, context: RouteCtx) {
  ...
  if (item.status !== "PHYSICAL_IN_PROGRESS") {
