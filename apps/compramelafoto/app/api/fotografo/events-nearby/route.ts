...
import { EventMemberRole, EventVisibility, Role } from "@/lib/prisma";
import { prisma } from "@/lib/prisma";
import { haversineDistanceMeters } from "@/lib/geo";
import { getR2PublicUrl } from "@/lib/r2-client";
import {
  resolveWorkingCoverageRadiusForEvents,
 ...
 * Eventos colaborativos visibles en descubrimiento (visibility=PUBLIC) dentro del radio del perfil.
...
export async function GET() {
  try {
    ...
    const profile = await prisma.user.findUnique({
      where: { id: user.id },
      select: { latitude: true, longitude: true, workingCoverageRadiusKm: true },
    });
    ...
    const radiusKm = resolveWorkingCoverageRadiusForEvents(profile?.workingCoverageRadiusKm);
    ...
    if (!hasLocation) {
      return NextResponse.json({
        events: [],
        noLocation: true,
        radiusKm: radiusKm ?? WORKING_COVERAGE_DEFAULT_KM,
        unlimited,
      });
    }
    ...
    const withDistance = events
      ...
      .filter((e) => unlimited || e.distanceKm <= radiusKm!)
    ...
    return NextResponse.json({
      events: list,
      radiusKm: unlimited ? null : radiusKm,
      unlimited,
    });
  } catch (err: any) {
