...
import { normalizeWorkingCoverageRadiusKm } from "@/lib/photographer/working-coverage-radius";
...
export async function PATCH(req: Request) {
  try {
    ...
    if (body.workingCoverageRadiusKm !== undefined) {
      const raw = body.workingCoverageRadiusKm;
      if (raw === null || raw === "unlimited" || raw === "") {
        updateData.workingCoverageRadiusKm = null;
      } else {
        const n = parseInt(String(raw), 10);
        updateData.workingCoverageRadiusKm = normalizeWorkingCoverageRadiusKm(n);
      }
    }
    ...
    if (err?.message?.includes("Unknown argument") || err?.message?.includes("Unknown field")) {
      return NextResponse.json(
        { 
          error: "Error: Los campos de personalización no existen en la base de datos. Por favor ejecuta: npx prisma migrate dev --name add_photographer_customization",
          detail: err?.message 
        },
        { status: 500 }
      );
