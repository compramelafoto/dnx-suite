...
export async function GET(
  ...
  try {
    ...
    const rosterDistinct = await prisma.albumStudentRosterEntry.findMany({
      ...
      distinct: ["albumId", "level", "shift", "courseName", "division"],
    });

    const rosterCourseSlotCountByAlbum = new Map<number, number>();
    for (const row of rosterDistinct) {
      rosterCourseSlotCountByAlbum.set(row.albumId, (rosterCourseSlotCountByAlbum.get(row.albumId) ?? 0) + 1);
    }

    const { albums: albumRows, ...schoolRest } = school;
    const albums = albumRows.map((a) => {
      const { _count, ...rest } = a;
      return {
        ...rest,
        photoCount: _count.photos,
        rosterCourseSlotCount: rosterCourseSlotCountByAlbum.get(a.id) ?? 0,
      };
    });

    return NextResponse.json({ ...schoolRest, albums });
  } catch (err: any) {
