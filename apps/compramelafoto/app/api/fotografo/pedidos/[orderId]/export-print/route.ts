 ...
 * Requiere fotógrafo del álbum o admin.
 */
...
async function buildPdf(
  order: {
    ...
    totalCents: number;
    buyerEmail: string;
    items: { finish?: string | null; size?: string | null; quantity: number }[];
  },
  ...
  draw(`Nombre: ${buyer.name || "-"}`, 12);
  draw(`Email: ${buyer.email || "-"}`, 12);
  draw(`WhatsApp / teléfono: ${buyer.phone || "-"}`, 12);
  y -= 10;
  ...
  try {
    ...
    const order = await prisma.order.findUnique({
      ...
      include: {
        ...
        items: {
          where: { productType: { in: ["PRINT", "FRAME"] } },
    ...
    if (!order || (!isAdmin && order.album?.userId !== user.id)) {
    ...
    let mirrorPhone: string | null = null;
    if (!buyer.phone) {
      const mirror = await prisma.printOrder.findFirst({
        where: { tags: { has: `ALBUM_ORDER:${order.id}` } },
        select: { customerPhone: true, customerName: true },
      });
      mirrorPhone = mirror?.customerPhone?.trim() || null;
      if (!buyer.name && mirror?.customerName?.trim()) {
        buyer.name = mirror.customerName.trim();
      }
      if (!buyer.phone && mirrorPhone) {
        buyer.phone = mirrorPhone;
      }
    }

    const customerLabel = safeName(buyer.name || buyer.email || "Cliente");
    const baseFolder = `Pedido-Album-${pad(order.id)} - ${customerLabel}`;
    const lines: string[] = [
    ...
    const pdfBytes = await buildPdf(order, buyer, (s, a) => getProductName(s, a));
