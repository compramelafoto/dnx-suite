...
/** Referencia lógica del carrito de álbum en PrintOrderItem.fileKey (no es key de R2). */
const PRINT_ORDER_FILE_KEY_PHOTO_REF = /^photo:(\d+)$/i;
...
async function buildPdf(
  ...
  draw(`Nombre: ${buyer.name || "-"}`);
  draw(`Email: ${buyer.email || "-"}`);
  draw(`WhatsApp / teléfono: ${buyer.phone || "-"}`);
  y -= 10;
...
export async function GET(
  ...
  try {
    ...
    if (linkedAlbumOrderId != null && Number.isFinite(linkedAlbumOrderId)) {
      const albumOrder = await prisma.order.findUnique({
        where: { id: linkedAlbumOrderId },
        select: {
          buyerEmail: true,
          buyerName: true,
          buyerPhone: true,
      ...
      if (albumOrder) {
        const resolved = formatBuyerContactForPhotographer(albumOrder, albumOrder.buyerUser);
        buyer = {
          name: buyer.name || resolved.name,
          email: buyer.email || resolved.email,
          phone: buyer.phone || resolved.phone,
        };
      }
    }

    const customer = safeName(buyer.name || buyer.email || "Cliente");
    ...
    lines.push(`WhatsApp / teléfono: ${buyer.phone || "-"}`);
    ...
    const photoIdsForR2 = new Set<number>();
    for (const it of order.items) {
      const m = String(it.fileKey).match(PRINT_ORDER_FILE_KEY_PHOTO_REF);
      if (m) {
        const pid = Number(m[1]);
        if (Number.isFinite(pid)) photoIdsForR2.add(pid);
      }
    }
    const photosForExport =
      photoIdsForR2.size > 0
        ? await prisma.photo.findMany({
            where: { id: { in: [...photoIdsForR2] } },
            select: { id: true, originalKey: true },
          })
        : [];
    const originalKeyByPhotoId = new Map(
      photosForExport.map((p) => [p.id, p.originalKey] as const)
    );
    ...
    for (const it of order.items) {
      ...
      let sourceKeyForR2 = it.fileKey;
      const photoRef = String(it.fileKey).match(PRINT_ORDER_FILE_KEY_PHOTO_REF);
      if (photoRef) {
        const pid = Number(photoRef[1]);
        const resolved = Number.isFinite(pid) ? originalKeyByPhotoId.get(pid) : undefined;
        if (resolved) sourceKeyForR2 = resolved;
      }
      const r2Key = urlToR2Key(sourceKeyForR2);
    ...
    // PDF (sin pdfkit)
    const pdfBytes = await buildPdf(order, buyer, getProductName, displayOrderId);
