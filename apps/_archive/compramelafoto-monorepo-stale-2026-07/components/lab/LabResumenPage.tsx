...
export default function LabResumenPage({
  lab,
  handler,
}: {
  lab: Lab;
  ...
  // Cargar precios del laboratorio
  useEffect(() => {
  ...
  function handleContinue() {
    router.push(`/l/${handler}/imprimir/datos`);
  }
          ...
          <div className="container-custom">
            <div className="max-w-4xl mx-auto space-y-8" style={{ wordBreak: "normal", overflowWrap: "normal" }}>
                    ...
                    {summaryBySizeAndFinish.map((entry, idx) => (
                      ...
                      >
                        <span className="text-sm text-[#6b7280]">
                          Fotografía - {entry.size} - {entry.finish === "BRILLO" ? "Brillo" : entry.finish === "MATE" ? "Mate" : entry.finish}
                        </span>
              ...
              <div className="flex justify-between pt-6">
                <Button
                  variant="secondary"
                  onClick={() => router.push(`/l/${handler}/imprimir`)}
                >
                  ← Modificar pedido
                </Button>
                <Button
