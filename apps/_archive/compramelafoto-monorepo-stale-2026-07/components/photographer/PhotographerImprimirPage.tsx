...
function calculateFinalUnitPrice(
  ...
  const feePercent = pricing.platformCommissionPercent ?? 0;
  const result = totalFromBase(Math.round(discountedPrice), feePercent);
  return Number.isFinite(result) ? result : 0;
}

export default function PhotographerImprimirPage({
  photographer,
  handler,
}: {
  ...
  // Calcular totales (aplicar margen de ganancia del fotógrafo)
  const totals = useMemo(() => {
    ...
    for (const item of items) {
      ...
      const discountedPrice = basePrice * (1 - discountPercent / 100);
      const labUnitPrice = calculateFinalUnitPrice(item.size, sizeQty, labPricing);

      totalBase += basePrice * item.quantity;
      totalLabAfterDiscount += Math.round(discountedPrice) * item.quantity;
      totalFinal += labUnitPrice * item.quantity;
    }

    return {
      totalBase,
      totalFinal,
      totalDiscount: totalBase - totalLabAfterDiscount,
    };
    ...
    async function loadPricing() {
      try {
        ...
        const products = Array.isArray(data.products)
          ? data.products.filter((p: LabProduct) => p.isActive !== false)
          : [];
  ...
  useEffect(() => {
    ...
    setItems((prevItems) => {
      if (prevItems.length > 0) {
        ...
        sessionStorage.setItem("orderItems", JSON.stringify(updatedExistingItems));
  ...
  async function handleFilesSelected(files: FileList) {
    ...
    try {
      ...
      setPhotos((prev) => {
        ...
        sessionStorage.setItem("uploadedPhotos", JSON.stringify(updated));
        return updated;
      });
      setShowUploadZone(false);
    } catch (err: any) {
  ...
  function handleContinue() {
    ...
    router.push(`/${handler}/imprimir/resumen`);
  }
          ...
          <div className="container-custom">
            <div className="max-w-4xl mx-auto space-y-8" style={{ wordBreak: "normal", overflowWrap: "normal" }}>
              ...
              >
                ...
                >
                  Usá la lista de precios del fotógrafo y elegí tamaño, acabado y cantidad. Aplicá la misma configuración a todas las fotos o personalizá cada una.
                </p>
                <p
                  style={{
                    fontSize: "13px",
                ...
                >
                  La impresión se realiza a cargo del fotógrafo. Selección de laboratorio próximamente.
                </p>
              </div>
                  ...
                  <div className="bg-[#f6f6f6] border border-[#e5e7eb] rounded-xl p-4" style={{ borderRadius: "12px" }}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                      {productsLoaded && labProducts.length > 0 && (() => {
                        // Agrupar productos por nombre genérico (excluir Fotos Carnet y Polaroid, solo para su flujo dedicado)
                        const productsGrouped = labProducts.reduce((acc, p) => {
                          const productName = p.name.split(' - ')[0].trim();
                          if (isCarnetOrPolaroidProduct(productName)) return acc;
                          if (!acc[productName]) {
                            acc[productName] = [];
                          }
                  ...
                  <div className="space-y-6">
                    {items.map((item, index) => {
                      ...
                      if ((item.productId || item.productName) && labProducts.length > 0) {
                        // Buscar el producto que tenga el nombre + tamaño actual
                      ...
                      const discountedPrice = basePrice * (1 - discountPercent / 100);
                      const labUnitPrice = Math.round(discountedPrice);
                      const feePercent = labPricing.platformCommissionPercent ?? 0;
                      const finalUnitPrice = Math.round(labUnitPrice * (1 + feePercent / 100));
                  ...
                  <Card className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[#1a1a1a]">{formatARS(totals.totalBase)}</span>
                      </div>
