...
export default function ClientAlbumView({
  ...
  return (
    <main className="select-none" style={{ padding: 24 }} data-protected="true">
      ...
      {Array.isArray(album.publicVisiblePacks) &&
      album.publicVisiblePacks.length > 0 ? (
        <section className="mb-6">
          <div className="rounded-xl border border-[#e5e7eb] bg-white p-4 md:p-5 space-y-4">
            <div>
              <h2 className="text-lg font-semibold text-[#1a1a1a]">
                Packs disponibles
              </h2>
              <p className="text-sm text-[#6b7280] mt-1">
                Vista informativa en prueba interna. La compra de packs estará disponible próximamente.
              </p>
            </div>
            <div className="grid gap-3 md:grid-cols-2">
              {album.publicVisiblePacks.map((pack) => (
                <article
                  key={pack.id}
                  className="rounded-lg border border-[#e5e7eb] bg-[#fafafa] p-4 space-y-2"
                >
                  ...
                  <ul className="text-xs text-[#4b5563] space-y-1">
                    <li>
                      Fotos incluidas:{" "}
                      {pack.includedPhotoCount != null
                        ? pack.includedPhotoCount
                        : "Sin límite fijo"}
                    </li>
                    <li>
                      Requiere selección: {pack.requiresSelection ? "Sí" : "No"}
                    </li>
                    <li>
                      Requiere diseño: {pack.requiresDesign ? "Sí" : "No"}
                    </li>
                  </ul>
                  <button
                    type="button"
                    disabled
                    className="mt-2 w-full rounded-md border border-[#d1d5db] bg-[#f3f4f6] px-3 py-2 text-sm text-[#6b7280] cursor-not-allowed"
                  >
                    {pack.previewOnly
                      ? "Este pack todavía está en prueba"
