...
type AlbumModeValue = (typeof albumModeOptions)[number]["value"];

export default function DashboardAlbumDetailPage() {
  const router = useRouter();
  const params = useParams();
  const albumId = params?.albumId ? parseInt(String(params.albumId)) : null;
  ...
  const [configTab, setConfigTab] = useState<
    "fotos" | "ventas" | "productos-packs" | "preventa"
  >("fotos");
  const [albumMode, setAlbumMode] = useState<AlbumModeValue>("SIMPLE");
  ...
  const currentAlbumMode = useMemo(
    () =>
      albumModeOptions.find((option) => option.value === albumMode) ??
      albumModeOptions[0],
    [albumMode],
  );
  ...
  async function saveAlbumMode() {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(data?.error || "No se pudo guardar el modo del álbum");
      }
      const nextMode =
        albumModeOptions.find((option) => option.value === data?.mode)?.value ??
        albumMode;
      setAlbumMode(nextMode);
      setAlbum((previous) =>
        previous ? { ...previous, mode: nextMode } : previous,
      );
    } catch (err: any) {
      setError(err?.message || "Error guardando el modo del álbum");
    } finally {
      ...
      <section className="py-12 md:py-16 bg-white min-h-screen">
        <div className="container-custom">
          ...
          <Card className="space-y-3">
            <h2 className="text-lg font-medium text-[#1a1a1a]">Modo del álbum</h2>
            <p className="text-sm text-[#6b7280]">
              Definí el tipo de álbum para ordenar mejor la configuración y preparar los próximos flujos.
            </p>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-[minmax(0,20rem)_auto] md:items-end">
              <label className="flex flex-col gap-1">
                <span className="text-sm font-medium text-[#1a1a1a]">Tipo de álbum</span>
                <select
                  value={albumMode}
                  onChange={(event) =>
                    setAlbumMode(event.target.value as AlbumModeValue)
                  }
              ...
              <Button
                variant="secondary"
                size="sm"
                onClick={saveAlbumMode}
                disabled={albumModeSaving}
              >
                {albumModeSaving ? "Guardando…" : "Guardar modo"}
              </Button>
            </div>
            <p className="text-xs text-[#6b7280]">{currentAlbumMode.description}</p>
          </Card>
          ...
          <Tabs
            tabs={[
              ...
              { id: "productos-packs", label: "Productos y packs" },
              { id: "preventa", label: "Pre-venta" },
            ]}
            activeTab={configTab}
            onTabChange={(id) =>
              setConfigTab(
                id as "fotos" | "ventas" | "productos-packs" | "preventa",
              )
            }
          >
            ...
            {configTab === "productos-packs" && album && (
              <div className="space-y-4">
                <Card className="space-y-3">
                  <h2 className="text-lg font-medium text-[#1a1a1a]">Productos y packs</h2>
                  <p className="text-sm text-[#6b7280]">
                    Desde acá podés configurar qué querés vender en este álbum: fotos sueltas, packs digitales, productos impresos o carpetas escolares con diseño.
                  </p>
                  <p className="text-sm text-[#6b7280]">
                    Estas sugerencias dependen del tipo de álbum seleccionado. Podés cambiar el modo del álbum desde la sección Resumen/Configuración.
                  </p>
                  <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                    Estos packs todavía no se muestran al cliente. Esta sección está en preparación para el nuevo flujo de venta por packs.
                  </div>
                </Card>
                <AlbumPacksSection albumId={album.id} albumMode={albumMode} />
              </div>
            )}
