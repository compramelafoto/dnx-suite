...
export default async function Page({ params }: { params: { id?: string } | Promise<{ id?: string }> }) {
  ...
  try {
    const rawPacks = await prisma.$queryRaw<
      Array<{
        id: string;
        name: string;
        description: string | null;
    ...
    >(
      Prisma.sql`SELECT "id", "name", "description", "price", "includedPhotoCount", "requiresSelection", "requiresDesign", "availabilityPhase", "isActive", "createdAt" FROM "AlbumPack" WHERE "albumId" = ${album.id} ORDER BY "createdAt" ASC`,
    );

    let albumMode: string | null = null;
    try {
      const modeRows = await prisma.$queryRaw<Array<{ mode: string | null }>>(
        Prisma.sql`SELECT "mode"::text AS "mode" FROM "Album" WHERE "id" = ${album.id} LIMIT 1`,
      );
      albumMode = modeRows[0]?.mode ?? null;
    } catch {
      albumMode = null;
    }

    publicVisiblePacks = getPublicVisiblePacks({
      packs: rawPacks,
      hasPhotos,
      albumMode,
      isOwnerOrAdminPreview: isOwner || isAdmin,
    });
  } catch {
    publicVisiblePacks = [];
  }
