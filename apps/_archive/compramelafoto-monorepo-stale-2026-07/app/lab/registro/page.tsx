...
export default function LabRegisterPage() {
  ...
  // Cerrar modal de términos con ESC
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/register-lab", {
        method: "POST",
      ...
      // El endpoint ya crea el usuario y el lab en una transacción
      if (data.pendingApproval) {
        setError(null);
        // Mostrar mensaje de éxito con advertencia sobre aprobación
        // El mensaje se muestra en el catch si hay error, pero si es éxito, redirigimos
        router.push("/lab/login?registered=pending");
        return;
      }

      // Redirigir al login
      router.push("/lab/login?registered=true");
    } catch (err: any) {
      ...
      <div className="container-custom">
        <div className="max-w-4xl mx-auto space-y-8" style={{ wordBreak: "normal", overflowWrap: "normal" }}>
          ...
          >
            ...
            >
              Registrate como laboratorio para comenzar
            </p>
            <Card className="bg-amber-50 border border-amber-200 max-w-2xl mx-auto mt-4">
            ...
            >
              Creá tu cuenta de laboratorio con email y contraseña o usá tu cuenta de Google para registrarte rápidamente.
            </p>
          </div>
          ...
          <Card className="space-y-6">
            ...
            <div className="text-center pt-4 border-t border-[#e5e7eb]">
              <p className="text-sm text-[#6b7280]">
                ¿Ya tenés cuenta?{" "}
                <Link href="/lab/login" className="text-[#c27b3d] hover:underline">
                  Iniciar sesión
                </Link>
              </p>
            </div>
