...
import { AlbumMode, Role, PrintPricingSource } from "@prisma/client";
...
export async function GET(
  req: NextRequest,
  { params }: { params: { albumId: string } | Promise<{ albumId: string }> }
) {
  try {
    ...
    const { albumId: albumIdParam } = await Promise.resolve(params);
...
export async function PATCH(
  ...
  try {
    ...
    const { title, location, eventDate, albumProfitMarginPercent, digitalPhotoPriceCents, printPricingSource, hiddenPhotosEnabled, hiddenSelfieRetentionDays, preCompraCloseAt, requireClientApproval, schoolId, type, mode } = body;
    ...
    if (mode !== undefined) {
      const validModes = new Set<AlbumMode>(Object.values(AlbumMode));
      if (!validModes.has(mode as AlbumMode)) {
        return NextResponse.json({ error: "Modo de álbum inválido" }, { status: 400 });
      }
