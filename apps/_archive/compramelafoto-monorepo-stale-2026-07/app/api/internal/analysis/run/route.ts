...
/**
 * Busca matches entre caras detectadas en una foto y selfies de interesados del mismo álbum
 * Crea FaceMatchEvent para cada match encontrado
 */
async function matchFacesWithInterested(
  ...
  try {
    // Obtener todos los interesados del álbum con faceId activo (no eliminados, no expirados)
    const now = new Date();
    ...
    if (interestedWithFaces.length === 0) {
      return; // No hay interesados con selfies en este álbum
    }

    // Buscar caras similares en la imagen usando Rekognition
    const faceMatches = await searchFacesByImage(new Uint8Array(imageBuffer));

    if (faceMatches.length === 0) {
      return; // No se encontraron caras similares
    }

    // Crear un mapa de faceId -> AlbumInterest para búsqueda rápida
    ...
    // Obtener las caras detectadas en esta foto para vincular con FaceDetection
    ...
    // Filtrar matches que correspondan a interesados del álbum
    ...
    for (const match of faceMatches) {
      ...
      if (interest && match.similarity && match.similarity >= 70) {
        // Threshold mínimo de 70% de similitud
    ...
    // Crear FaceMatchEvent para cada match (evitar duplicados)
    if (matchesToCreate.length > 0) {
      // Verificar si ya existen matches para evitar duplicados
      ...
      const newMatches = matchesToCreate.filter(
        (m) => !existingInterestIds.has(m.albumInterestId)
      );
      ...
      if (newMatches.length > 0) {
        await prisma.faceMatchEvent.createMany({
          data: newMatches.map((m) => ({
            ...
            notifiedAt: null, // Se notificará después
          })),
        });

        console.log("Face matches creados", {
          photoId,
          albumId,
          matchesCount: newMatches.length,
        });

        // Intentar enviar notificaciones de forma asíncrona (no bloquear si falla)
        if (newMatches.length > 0) {
          ...
          fetch(notifyUrl, {
            ...
            console.warn("Error llamando endpoint de notificaciones (no crítico):", fetchErr);
          });
        }
      }
    }
  } catch (err: any) {
    console.error("Error en matchFacesWithInterested:", {
      photoId,
      albumId,
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      try {
        ...
        const readErrorMsg = String(readErr?.message || readErr);
        console.error("Error leyendo imagen desde R2:", {
          photoId: photo.id,
          originalKey: photo.originalKey,
          error: readErrorMsg,
        });
        throw new Error(`Error leyendo imagen desde R2: ${readErrorMsg}`);
      }
      ...
      // Validar que el buffer leído desde R2 parece ser una imagen válida
      ...
      if (!isValidImageStart) {
        console.error("Buffer leído desde R2 no parece ser una imagen válida:", {
          photoId: photo.id,
          originalKey: photo.originalKey,
          bufferSize: imageBuffer.length,
          firstBytes: bufferStart.toString('hex'),
          contentType: r2Metadata?.contentType,
        });
        throw new Error(`Imagen corrupta: el buffer leído desde R2 no parece ser una imagen válida (inicio: ${bufferStart.toString('hex')})`);
      }
      ...
      try {
        ...
        // Remover TODOS los metadatos EXIF/ICC/XMP que pueden causar problemas con Google Vision
        ...
        // Crear un JPEG completamente limpio, sin metadatos, para máxima compatibilidad
        // Esto es crítico para evitar errores "DECODER routines::unsupported" en Google Vision API
        // Google Vision API requiere JPEGs estándar sin características avanzadas
        normalizedBuffer = await image
          .removeAlpha() // Remover canal alpha si existe
          .jpeg({ 
            quality: 90, // Reducir calidad ligeramente para asegurar compatibilidad
            mozjpeg: false, // NO usar mozjpeg - usar JPEG estándar para máxima compatibilidad
            progressive: false, // No usar progressive JPEG
            optimizeScans: false, // Desactivar optimizaciones que pueden causar problemas
          })
          .withMetadata({}) // Remover TODOS los metadatos (EXIF, ICC, XMP, etc.) - crítico para Google Vision
          .toBuffer();
        ...
        // Validación adicional: verificar que Sharp puede leer el buffer normalizado
        // Esto asegura que el JPEG es completamente válido antes de enviarlo a Google Vision
        try {
          const validationSharp = sharp(normalizedBuffer);
          const validationMeta = await validationSharp.metadata();
          if (!validationMeta.width || !validationMeta.height || validationMeta.format !== 'jpeg') {
            throw new Error(`JPEG normalizado no es válido (width: ${validationMeta.width}, height: ${validationMeta.height}, format: ${validationMeta.format})`);
          }
        } catch (validationErr: any) {
          console.error("Error validando JPEG normalizado:", {
            photoId: photo.id,
            error: validationErr?.message,
            bufferSize: normalizedBuffer.length,
            firstBytes: normalizedBuffer.slice(0, 8).toString('hex'),
          });
          throw new Error(`JPEG normalizado no es válido: ${validationErr?.message}`);
        }
      ...
      try {
        ...
        // Validar que el buffer es válido antes de enviarlo
        const jpegStartCheck = normalizedBuffer.slice(0, 2);
        if (jpegStartCheck[0] !== 0xFF || jpegStartCheck[1] !== 0xD8) {
          throw new Error(`Buffer normalizado no es un JPEG válido antes de enviar a Google Vision (inicio: ${jpegStartCheck.toString('hex')})`);
        }
        ...
        console.log("Calling Google Vision API", {
          ...
          bufferSize: normalizedBuffer.length,
          originalFormat: imageFormat,
          bufferStart: normalizedBuffer.slice(0, 4).toString('hex'),
          bufferEnd: normalizedBuffer.slice(-4).toString('hex'),
        });
        ...
        // Si es un error de decoder, registrar pero continuar con reconocimiento facial
        // Esto permite que al menos el reconocimiento facial se ejecute aunque OCR falle
        if (isDecoderError) {
          console.error("OCR failed due to decoder error, but continuing with face recognition", {
            photoId: photo.id,
            error: errorMsg,
          });
          // NO lanzar error aquí - continuar con reconocimiento facial
          ocrTokens = []; // Asegurar que está vacío
        } else if (isAuthError) {
          ...
          // Para otros errores, también continuar (podría ser un problema temporal)
          console.error("OCR failed with non-critical error, continuing with face recognition", {
            photoId: photo.id,
            error: errorMsg,
          });
          ocrTokens = [];
      ...
      if (faces.length > 0) {
        ...
        // Matching automático con selfies de interesados
        try {
          await matchFacesWithInterested(photo.id, photo.albumId, normalizedBuffer);
        } catch (matchErr: any) {
          // No fallar el análisis si el matching falla
          console.error("Error en matching automático con interesados:", {
            photoId: photo.id,
