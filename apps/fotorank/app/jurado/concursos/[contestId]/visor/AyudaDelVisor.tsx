...
const PASOS: Array<{ titulo: string; texto: string }> = [
  ...
  {
    titulo: "Cuatro notas por foto",
    texto:
      "Cada fotografía se califica con cuatro criterios, del 1 al 10. Al elegir una nota " +
      "el visor pasa solo al criterio siguiente, así que una foto son cuatro teclas.",
  },
  {
...
const TECLAS: Array<{ tecla: string; hace: string }> = [
  { tecla: "→", hace: "Foto siguiente, sin calificar" },
  { tecla: "←", hace: "Foto anterior, sin calificar" },
  { tecla: "1 … 9", hace: "La nota del criterio en el que estás" },
  { tecla: "0", hace: "Vale 10" },
  { tecla: "Tab", hace: "Criterio siguiente; en el último, pasa de foto" },
  { tecla: "⇧ Tab", hace: "Criterio anterior; en el primero, vuelve de foto" },
  { tecla: "F", hace: "Pantalla completa: esconde todo menos la foto" },
  { tecla: "Esc", hace: "Salir de pantalla completa" },
];
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: colores.panel, color: colores.tinta, border: `1px solid ${colores.linea}` }}
      ...
      >
          ...
          {PASOS.map((p, i) => (
            <li key={p.titulo} className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm leading-relaxed" style={{ color: colores.suave }}>
                  {p.texto}
                </p>
              </div>
            </li>
        ...
        <table className="mt-3 w-full text-left text-sm">
          <tbody>
            {TECLAS.map((t) => (
              <tr key={t.tecla} style={{ borderTop: `1px solid ${colores.linea}` }}>
                <td className="w-28 py-2 pr-4 align-top">
                  <kbd
                    className="inline-block px-2 py-1 font-mono text-xs"
                    style={{ background: colores.chip, border: `1px solid ${colores.linea}` }}
                  >
                    {t.tecla}
                  </kbd>
                </td>
        ...
        <p className="mt-2 text-sm leading-relaxed" style={{ color: colores.suave }}>
          <b style={{ color: colores.tinta }}>Me faltan</b> son las que todavía no tienen las
          cuatro notas, hayas empezado o no.{" "}
          <b style={{ color: colores.tinta }}>A medias</b> son las que empezaste y dejaste por
          la mitad — esas son las que conviene revisar antes de enviar, porque no cuentan.
          Cambiar de filtro rearma la lista; mientras calificás, ninguna foto se te va de la
          pantalla sola.
        </p>
      </div>
    </div>
