...
const ELEGIDO = { background: "#e0a061", border: "1px solid #e0a061", color: "#1b1917", fontWeight: 600 };
...
export function BotoneraDeNota({
  ...
  const valores =
    forma === "SI_NO"
      ? [0, 1]
      : Array.from({ length: criterio.max - criterio.min + 1 }, (_, k) => criterio.min + k);
...
function CampoDeNota({
  ...
  function confirmar() {
    ...
    if (texto.trim() !== "" && Number.isInteger(n) && n >= criterio.min && n <= criterio.max) {
      onElegir(n);
    } else {
      setTexto(puesta != null ? String(puesta) : "");
    }
  ...
  return (
    <input
      ...
      style={{ background: colores.chip, border: `1px solid ${colores.linea}`, color: "inherit" }}
    />
  );
}
