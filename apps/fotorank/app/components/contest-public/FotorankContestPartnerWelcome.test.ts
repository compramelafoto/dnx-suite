...
describe("FotoRank CONTEST welcome wiring", () => {
  it("página monta wrapper + loader; partnerGroups=[] intacto; no home", () => {
    ...
    assert.match(page, /partnerGroups=\{\[\]\}/);
  ...
  it("landing pública usa contest-public (no public-ui)", () => {
    const landing = readFileSync(
      join(here, "../../concursos/[slug]/ContestPublicLanding.tsx"),
      "utf8",
    );
    assert.match(landing, /contest-public/);
    assert.match(landing, /ContestPartnersSection/);
  });
});
