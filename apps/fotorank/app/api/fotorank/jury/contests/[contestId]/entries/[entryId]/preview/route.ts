...
import { JuryError, getJuryPreviewAccess } from "../../../../../../../../lib/fotorank/jury";
...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  try {
    const data = await getJuryPreviewAccess({
    ...
    console.error("[jury preview]", err);
    return NextResponse.json({ error: { code: "INTERNAL", message: "Error al firmar preview." } }, { status: 500 });
  }
}
