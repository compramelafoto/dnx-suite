...
export function buildChatGptRulesPrompt(config: ContestRulesConfiguration): string {
  ...
  return [
    ...
    `- Participación: modalidad ${part.pricingMode}; ABIERTA (residencyRequired=${String(part.residencyRequired)}); edad mínima ${part.minAge}; obras ${part.maxEntriesPerRegistration}; categorías ${part.maxCategoriesPerRegistration}`,
    `- Territorio de la FOTOGRAFÍA (no del domicilio): ${identity.territoryScope ?? identity.province ?? "—"}`,
    `- Período de captura: ${schedule.captureWindowStartsAt ?? "—"} → ${schedule.captureWindowEndsExclusiveAt ?? "—"} (${schedule.timezone})`,
    `- Menores: autorización adulto ${String(part.adultAuthorizationRequired)}`,
    `- Categorías: ${config.categories.map((c) => `${c.name} [${c.slug}; device=${c.deviceType}; membership=${c.membershipRestriction ?? "none"}]`).join("; ")}`,
    `- ARGRA: solo si membershipRestriction=ARGRA; número privado; verificación manual; PENDING_INSTITUTIONAL_APPROVAL`,
    `- Archivo: MIME ${config.file.supportedMimeTypes.join(", ")}; sin límites reglamentarios inventados`,
    `- Metadatos: EXIF/GPS/fecha/dispositivo según JSON (GPS opcional; no publicar GPS ni ARGRA)`,
