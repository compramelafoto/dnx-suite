/**
 * Cleanup SFEF07B — staging only, dry-run por defecto.
 * APPLY=1 para borrar.
 */
...
async function main() {
  ...
  const users = await prisma.user.findMany({
    where: {
      email: { endsWith: "@fotorank.test", startsWith: "sfef07b-" },
    },
    select: { id: true, email: true },
  });
  const judges = await prisma.fotorankJudgeAccount.findMany({
    where: { email: { endsWith: "@fotorank.test", startsWith: "sfef07b-" } },
    select: { id: true, email: true },
  });
  ...
  if (judgeIds.length) {
    await prisma.fotorankJudgeInvitation.deleteMany({
      where: { contestId: contest.id, email: { startsWith: "sfef07b-" } },
    });
