        ...
        <div className="mt-14 space-y-12">
          {secciones.map((seccion) => (
            <section key={seccion.titulo}>
              <h2 className="text-xl font-extrabold leading-snug">{seccion.titulo}</h2>
              ...
              {seccion.puntos ? (
                <ul
                  ...
                  style={{ color: "var(--slf-tinta-suave)", listStyleType: "disc" }}
                >
...
function conNegritas(texto: string) {
  return texto.split(/(\*\*[^*]+\*\*)/g).map((trozo, i) =>
    trozo.startsWith("**") && trozo.endsWith("**") ? (
      <strong key={i} className="font-extrabold" style={{ color: "var(--slf-tinta)" }}>
        {trozo.slice(2, -2)}
      </strong>
    ) : (
      trozo
