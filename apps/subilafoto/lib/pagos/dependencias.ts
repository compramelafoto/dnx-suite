...
import type { ConnectDeps, ConnectStateRecord } from "@repo/payments/mercado-pago-connect";
import { leerConfigDePagos } from "./config";

/**
 * Clave maestra para cifrar el verificador PKCE.
 *
 * Es **la misma variable que usa la bóveda de credenciales** de toda la suite, a
 * propósito: dos claves distintas para el mismo flujo serían dos cosas que rotar y dos
 * formas de perder acceso a lo cifrado.
 */
 ...
 *
 * Es el mismo que FotoOffice salvo el origen que se anota en la credencial. Las tablas
 * son compartidas —`DnxFinancialIdentity`, `DnxPaymentAccount`, la bóveda— y lo que
 * separa a un producto de otro es el `organizationRef` con prefijo.
 */
function almacenDeConexion(): ConnectDeps["store"] {
  return {
    ...
    async findStateByHash(stateHash) {
      ...
      // La referencia del vendedor se recupera de la identidad, que es donde vive.
    ...
    async saveCredential(record) {
      ...
      const saved = await vault.encryptMercadoPagoCredential({
        ...
        payload: {
          ...
          origin: "subilafoto_seller_oauth",
...
/** Dependencias reales del flujo de conexión. El producto lo pone quien llama. */
export function dependenciasDeConexion(): Omit<ConnectDeps, "product"> {
  return {
    config: leerConfigDePagos(),
    mpClient: createLiveMercadoPagoOAuthHttpClient(),
    masterKeyBase64: claveMaestra(),
    store: almacenDeConexion(),
  };
}
