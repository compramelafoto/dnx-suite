# Dossier de funcionalidades originales — ComprameLaFoto

**Obra:** ComprameLaFoto (plataforma integrante de DNX Suite)
**Tipo:** Programa de computación — obra inédita
**Titular:** `[COMPLETAR: nombre, CUIT, nacionalidad, domicilio, % de titularidad de cada autor]`
**Fecha del snapshot:** `[COMPLETAR al generar el paquete]`
**Destino:** Depósito en custodia de obra inédita — Dirección Nacional del Derecho de Autor (DNDA)
**Versión del documento:** 1.0 — 3 de septiembre de 2026
...
| Campo | Valor |
|-------|-------|
| Denominación | ComprameLaFoto |
| Suite | DNX Suite (monorepo Turborepo + pnpm workspaces) |
| Aplicaciones hermanas | FotoRank, Info Spot, Fotoffice, Clickaton |
| Naturaleza | Plataforma web de venta de fotografía de eventos, escolar y deportiva |
| Stack | Next.js 16 (App Router), TypeScript, PostgreSQL + Prisma, Cloudflare R2, AWS Rekognition, Sharp, MercadoPago, Resend |
| Escala del código | 455 modelos de datos; ~751 rutas y endpoints en la app; ~834 módulos de biblioteca |
| Ruta en repositorio | `dnx-suite/apps/compramelafoto` + `dnx-suite/packages/db` |

### Titularidad — a completar antes del depósito

El formulario de la DNDA exige, por cada autor: nombre y apellido, CUIT/CUIL, nacionalidad, domicilio y **porcentaje de titularidad**. Si en el código intervino algún tercero (socio, freelance, agencia), definirlo ahora: corregir la titularidad después del depósito es considerablemente más difícil que declararla bien la primera vez.
...
/codigo-fuente/
    apps/compramelafoto/                 Sin node_modules, .next, .turbo ni secretos
    packages/db/prisma/schema.prisma     455 modelos
    packages/                            Paquetes compartidos utilizados
/anexos/
...
- [ ] Definir autores y porcentajes de titularidad
- [ ] Decidir alcance: ¿solo ComprameLaFoto o DNX Suite completa? (mismo arancel)
