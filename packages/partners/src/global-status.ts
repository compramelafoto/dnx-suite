...
function flagKeyForPlacement(placementKey: string): string {
  switch (placementKey) {
    ...
    case "FOTORANK_HOME_MARQUEE":
      return "FOTORANK_HOME_MARQUEE_ENABLED";
    case "FOTORANK_CONTEST_MARQUEE":
      return "FOTORANK_CONTEST_MARQUEE_ENABLED";
    case "CLICKATON_EVENT_WELCOME":
      return "CLICKATON_PARTNER_WELCOME_ENABLED";
    case "CLICKATON_HOME_MARQUEE":
      return "CLICKATON_HOME_MARQUEE_ENABLED";
    case "CLICKATON_EVENT_MARQUEE":
      return "CLICKATON_EVENT_MARQUEE_ENABLED";
    case "INFOSPOT_HOME_WELCOME":
    case "INFOSPOT_HOME_MARQUEE":
      return "INFOSPOT_PARTNER_ADS_ENABLED";
    case "CLF_ALBUM_WELCOME":
      return "CLF_PARTNER_ALBUM_WELCOME_ENABLED";
    default:
      return "—";
  }
}
...
export function computePartnerGlobalHealth(
  ...
  if (anyFlagOn && input.campaigns.total > 0 && input.sync.failed === 0) {
    return "HEALTHY";
  }
...
export function centralPlatformStatusPathForApp(
  application: PartnerGlobalStatusApplication,
): string {
  switch (application) {
    case "CLICKATON":
      return "/admin/sponsors/estado-global/clickaton";
    case "FOTO_RANK":
...
export function resolveDnxPartnersCentralPlatformUrl(
  application: PartnerGlobalStatusApplication,
  envValue?: string | null,
): string {
  const origin = assertSafePartnersCentralAdminUrl(
    resolveDnxPartnersCentralAdminUrl(envValue),
  );
  const base = origin.replace(/\/admin\/sponsors\/?$/, "");
  return `${base}${centralPlatformStatusPathForApp(application)}`;
}
