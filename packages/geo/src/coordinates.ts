...
/** Alias histórico InfoSpot. */
export const hasUsableEventCoordinates = hasUsableCoordinates;

export function parseLatLon(
  latitude: unknown,
  longitude: unknown,
): Coordinates | null {
  const v = validateCoordinates(latitude, longitude);
  return v.ok ? v.coordinates : null;
}
