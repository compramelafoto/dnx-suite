 ...
 *
 * Nota: el schema local aún no incluye por completo DnxPartnerAsset /
 * DnxPartnerParticipationAsset / BenefitAccess.accessKey que el dominio
 * `@repo/partners` ya tipa. Los métodos de partner/participation institucionales
 * (Stage 04) sí alinean con schema + migración 20260807120000.
 */
...
export function createPrismaPartnersRepository(): PartnersRepository {
  return {
    ...
    async createClickEvent(input) {
      return prisma.dnxPartnerClickEvent.create({
        data: {
    ...
    async listClickEventsByPartner(partnerId) {
      return prisma.dnxPartnerClickEvent.findMany({
        where: { partnerId },
