...
async function loadMetrics(
  prisma: PrismaClient,
  application: PartnerGlobalStatusApplication,
  _mode: PartnerGlobalStatusLoadMode,
) {
  try {
    const [impressions, lastImp] = await Promise.all([
    ...
    let clicksNote: string | null = null;
    try {
      ...
      clicksNote = "Métrica no disponible en esta réplica.";
    }

    const lastActivityAt =
      [lastImp?.occurredAt, lastClickAt]
        .filter(Boolean)
        .sort((a, b) => (a! > b! ? -1 : 1))[0]
        ?.toISOString() ?? null;
