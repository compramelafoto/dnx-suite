import { CredentialVault, createPrismaCredentialStore } from "../index.js";
import { createLiveMercadoPagoOAuthHttpClient } from "../index.js";
import type { MpConnectConfig } from "./service.js";
import { sanitizeError } from "./log.js";
import { decideTokenUse } from "./token-freshness.js";

/**
 * Con quién cobra el conectado.
 *
 * En el modelo de dos vías el cobrador es el propio conectado —una institución en
 * FotoOffice, un vendedor en Subí la Foto—, así que hace falta **su** token y no el de la
 * plataforma. La vinculación por OAuth ya lo guardó cifrado en la bóveda.
 *
 * Vive en el paquete desde el 2026-09-14: lo único que cambiaba entre productos era cómo
 * se arma la referencia de organización, que ahora entra por parámetro.
 */
 ...
 */
export type ResolveCollectorInput = {
  /** A quién pertenece la cuenta, con el prefijo del producto. */
  organizationRef: string;
  /** El cliente de Prisma de la aplicación. El paquete no lo importa. */
  prisma: PrismaParaCollector;
  config: MpConnectConfig;
  /** Sólo para los registros, para saber qué producto falló. */
  productKey: string;
  now?: Date;
};

/** Lo mínimo del cliente de Prisma que este flujo necesita. */
export type PrismaParaCollector = {
  dnxFinancialIdentity: { findUnique: (args: never) => Promise<{ id: string } | null> };
  dnxPaymentAccount: {
    findFirst: (args: never) => Promise<{
      id: string;
...
export async function resolveCollector(entrada: ResolveCollectorInput): Promise<CollectorResult> {
  const { organizationRef, prisma, config, productKey } = entrada;
  const opciones = { now: entrada.now };

  const identity = await (prisma.dnxFinancialIdentity.findUnique as never as (a: unknown) => Promise<{ id: string } | null>)({
  ...
  const account = await (prisma.dnxPaymentAccount.findFirst as never as (a: unknown) => Promise<{ id: string; providerUserId: string | null; credentialReference: string | null; status: string } | null>)({
  ...
  try {
    ...
    console.error(`[${productKey}][collector] no se pudo leer la credencial`, {
  ...
  try {
    ...
    console.error(`[${productKey}][collector] MercadoPago rechazó el refresco`, {
      detalle: sanitizeError(error),
    });
    return problema(
      "REFRESH_FAILED",
  ...
  try {
    ...
    await (prisma.dnxPaymentAccount.update as never as (a: unknown) => Promise<unknown>)({
      where: { id: account.id },
      data: { credentialReference: nueva.id },
    });
  } catch (error) {
    console.error(`[${productKey}][collector] no se pudo guardar la credencial renovada`, {
      detalle: sanitizeError(error),
    });
