/**
 * Conectar una cuenta de MercadoPago de un tercero, para toda la suite.
 *
 * Escrito primero dentro de FotoOffice y subido acá el 2026-09-14, cuando Subí la Foto
 * necesitó lo mismo. Lo único que cambia entre productos es **quién** conecta y con qué
 * capacidades; el flujo —PKCE, estado de un solo uso, bóveda de credenciales— es idéntico.
 *
 * Nota: hay dos flujos de conexión distintos y conviene no confundirlos. Este es "un
 * tercero conecta su cuenta para cobrar". `partner-onboarding/owner-oauth` es "la
 * plataforma conecta su propia cuenta", que responde otra pregunta.
 */
import {
...
/**
 * Configuración de la aplicación de MercadoPago que usa el producto.
 *
 * `redirectUri` tiene que coincidir **byte a byte** con la declarada en el panel de
 * MercadoPago: una barra final de más y el intercambio del código falla con un error
 * opaco que cuesta horas diagnosticar.
 */
...
/**
 * Qué producto de la suite está conectando.
 *
 * La tabla de estados OAuth es compartida por todo el monorepo, así que cada producto
 * tiene que identificarse: sin esto, el retorno de una conexión de un producto podría
 * completar la de otro.
 */
export type ConnectProduct = {
  /** Va en `DnxMercadoPagoOAuthState.productKey` y en `originApp` de la cuenta. */
  key: string;
  /** Cómo se llama en los mensajes de error que ve una persona. */
  nombre: string;
  /**
   * Capacidades de la cuenta conectada.
   *
   * `SPLIT_RECEIVER` es para el reparto 1:N. Con el modelo de dos vías
   * (`marketplace_fee`) el conectado es el **cobrador**, no un receptor de reparto.
   */
  capabilities: string[];
};
...
export type ConnectStateRecord = {
  ...
  expiresAt: Date;
  usedAt?: Date | null;
  /** A quién pertenece esta conexión, para poder volver a él en el callback. */
...
export async function startMpConnection(
  input: { organizationRef: string; userId: number; legalName?: string },
  deps: ConnectDeps,
...
export async function completeMpConnection(
  ...
  if (state.productKey !== deps.product.key) {
    // La tabla de estados es compartida con otros productos del monorepo.
    throw new ConnectError(
      "STATE_WRONG_PRODUCT",
      `El pedido de conexión no es de ${deps.product.nombre}.`,
    );
  }
  ...
  const account = await deps.store.upsertPaymentAccount({
    ...
    originApp: deps.product.key,
    capabilities: deps.product.capabilities,
    status: "ACTIVE",
    connectedAt: now,
